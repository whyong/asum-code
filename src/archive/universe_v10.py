"""
涌积态宇宙模型 v10.0 — 双通道相变驱动架构
基于 v9.0（T-009 最终参数），针对一项核心未解问题的定向升级：

  核心问题：v9.0 在 500000 帧后 C_var 仍只有 0.066（v8.0 达到 0.097），
  trough_adapt 的缓慢势垒降低（0.9999/帧）在 500000 帧内只降低 0.011，
  不足以触发真正的高位相变。系统陷入"低振幅持续振荡"的亚稳态陷阱。

  v10.0 的解决方案：双通道相变驱动（Dual-Channel Phase Transition Driver）

  通道一：动量加速势垒（Momentum-Accelerated Barrier）
    在 trough_adapt 触发期间，追踪 f_C2 的变化动量（移动平均斜率）。
    若 f_C2 在低谷期持续下降（负动量），说明系统正在深陷低谷，
    此时加速 Theta_barrier 的降低速率（decay_rate 从 0.9999 加速到 0.9990）。
    若 f_C2 开始回升（正动量），恢复正常速率（0.9999）。
    实现"越陷越深越加速"的自适应相变催化。

  通道二：低谷期失真增强（Trough Distortion Amplifier）
    在 trough_adapt 触发期间，向 k_noise 注入额外的失真增益（k_noise_boost）。
    增强失真算子的对称性破缺能力，让系统在低谷期更容易探索相空间。
    k_noise_boost 与 trough_adapt 同步触发/关闭，避免干扰高位稳定态。

  铁律对齐：
    通道一（动量加速）= 有损算子的"时间沉淀"加速（更快降低势垒 = 更快提取宏观趋势）
    通道二（失真增强）= 失真算子的"空间创造"增强（更强噪声 = 更大对称性破缺）
...
class Config:
    ...
    alpha_metric_C1 = 0.0   # C1 度规禁用（v9.0 T-003 确认）
    ...
    trough_fc2_thresh     = 0.20   # 恢复 v9.0 最优值
    trough_phi_thresh     = 25.0
    trough_decay_rate     = 0.9999   # 基础衰减率（动量加速时会加快）
    trough_theta_min      = 0.18

    # ── [v10.0 新增] 通道一：C_var 停滞检测加速势垒 ──────────────────
    # T-009 教训：C_var 停滞检测在图灵期初期容易误触发，禁用
    # 通道一的功能由 trough_adapt 基础衰减（0.9999）承担
    momentum_enabled      = False    # 禁用（T-009 确认误触发风险）
    momentum_window       = 5000
    momentum_decay_fast   = 0.9995
    momentum_decay_normal = 0.9999
    momentum_threshold    = 0.05
    momentum_cvar_min     = 0.03

    # ── [v10.0 新增] 通道二：低谷期失真增强 ──────────────────────────
    # 在 trough_adapt 触发期间，向 k_noise 注入额外增益
    distortion_boost_enabled = True
    distortion_boost_max     = 0.20  # 最大额外噪声增益（叠加到 k_noise=0.50 上）
    # 实际增益 = distortion_boost_max * (1 - f_C2 / trough_fc2_thresh)
    # f_C2 越低，增益越大（越陷越深，失真越强）
...
class AdaptiveTuner:
    ...
    def _check_rules(self, tick, phase, f_C2, f_C1, phi_net, c_var, delta_sig):
        ...
        # 规则2：C2 积累过慢（v9.0 弱化：仅在 Phi_net 极低时触发）
...
class PhysicsLayer:
    def __init__(self, cfg: Config):
        ...
        # [v10.0] 停滞检测缓冲区
        self._cvar_buf       = []   # 用于检测 C_var 停滞
        self._trough_active  = False  # 当前是否处于 trough_adapt 触发状态
        self.trough_momentum = 0.0    # 供观测层读取（正=增长，负/零=停滞）
        self.k_noise_eff     = cfg.k_noise  # 实际使用的噪声强度（含 boost）
    ...
    def step(self, tick: int = 0, c_var: float = 0.0, phase: str = 'CHAOTIC_EDGE'):
        ...
        # ── 步骤4：D 失真算子（含 v10.0 低谷期失真增强）────────────
        # [v10.0] 通道二：C_var 停滞时增强失真（与通道一同步，基于 trough_momentum）
        k_noise_eff = cfg.k_noise
        if cfg.distortion_boost_enabled and self.trough_momentum < 0:
            # trough_momentum 越负（停滞越深），增益越大
            stagnation_factor = min(1.0, max(0.0, -self.trough_momentum / 0.5))
            k_noise_eff = cfg.k_noise + cfg.distortion_boost_max * stagnation_factor
        ...
        if phase == 'TURING_GROWTH':
            ...
            # trough_adapt 触发条件（继承 v9.0）
            trough_condition = (cfg.trough_adapt_enabled
                                and f_C2_now < cfg.trough_fc2_thresh
                                and phi_net_now > cfg.trough_phi_thresh
                                and cfg.Theta_barrier > cfg.trough_theta_min)
            self._trough_active = trough_condition

            # 通道一：C_var 停滞检测（独立于 trough_condition，覆盖更广的亚稳态）
            # 在 TURING_GROWTH 期内，只要 C_var 停滞就触发加速
            if cfg.momentum_enabled:
                self._cvar_buf.append(c_var)
                if len(self._cvar_buf) > cfg.momentum_window:
                    self._cvar_buf.pop(0)

                if len(self._cvar_buf) >= cfg.momentum_window and c_var > cfg.momentum_cvar_min:
                    mid = cfg.momentum_window // 2
                    cvar_early = float(np.mean(self._cvar_buf[:mid]))
                    cvar_late  = float(np.mean(self._cvar_buf[mid:]))
                    # 停滞判断：C_var 增长 < momentum_threshold
                    # 额外保护：cvar_late 必须在合理范围内（不能是正在快速下降的峰值后）
                    # 且 cvar_early 和 cvar_late 都必须 > momentum_cvar_min（排除启动期）
                    cvar_stable = (cvar_early > cfg.momentum_cvar_min
                                   and cvar_late > cfg.momentum_cvar_min
                                   and cvar_late < cvar_early * (1 + cfg.momentum_threshold)
                                   and cvar_late > cvar_early * 0.80)  # 防止快速下降误判
                    if cvar_stable:
                        # C_var 停滞（在平台期振荡，没有显著增长也没有快速下降）→ 加速降低 Theta_barrier
                        self.trough_momentum = (cvar_late - cvar_early) / (cvar_early + 1e-9)
                        if cfg.Theta_barrier > cfg.trough_theta_min:
                            cfg.Theta_barrier = max(cfg.trough_theta_min,
                                                    cfg.Theta_barrier * cfg.momentum_decay_fast)
                    else:
                        self.trough_momentum = max(0.0, (cvar_late - cvar_early) / (cvar_early + 1e-9))
                else:
                    self.trough_momentum = 0.0
            else:
                self.trough_momentum = 0.0

            # trough_adapt 基础衰减（继承 v9.0，在 trough_condition 满足时触发）
            if trough_condition:
                cfg.Theta_barrier = max(cfg.trough_theta_min,
                                        cfg.Theta_barrier * cfg.trough_decay_rate)
        else:
            # 不在 TURING_GROWTH 期，清空停滞缓冲
            self._cvar_buf.clear()
            self.trough_momentum = 0.0
            self._trough_active = False


# ─────────────────────────────────────────────
# 四、观测层（继承 v9.0，新增 v10.0 指标）
# ─────────────────────────────────────────────
...
class ObservationLayer:
    def __init__(self, cfg: Config):
        ...
        self.history = {k: [] for k in [
            ...
            'trough_momentum', 'k_noise_eff',  # [v10.0 新增]
        ]}
        self._hd_buf = []
    ...
    def observe(self, phys: PhysicsLayer, tick: int):
        ...
        h['trough_momentum'].append(phys.trough_momentum)   # [v10.0]
        h['k_noise_eff'].append(phys.k_noise_eff)           # [v10.0]
        return phase, C_var
...
def render(phys: PhysicsLayer, obs: ObservationLayer, tick: int):
    ...
    for key, color, lbl in [
        ...
        ('trough_momentum',  '#9b59b6', '[v10] 低谷动量（负=加速）'),
    ...
    momentum_now = h['trough_momentum'][-1] if h['trough_momentum'] else 0.0
    fig.suptitle(
        f'涌积态宇宙 v10.0   第 {tick:05d} 帧   '
        f'C2占比={h["f_C2"][-1]:.3f}   Θ={theta_now:.4f}   '
        f'动量={momentum_now:+.4f}   '
...
def run():
    ...
    print("=" * 65)
    print("涌积态宇宙模型 v10.0 启动")
    print(f"  网格: {cfg.W}×{cfg.H}   dt={cfg.dt}   最大帧数={cfg.steps}")
    print(f"  [v8.0继承] alpha_metric={cfg.alpha_metric}  seed_freq={cfg.seed_freq}")
    print(f"  [v9.0继承] seed_adaptive={cfg.seed_adaptive}  "
          f"trough_adapt={cfg.trough_adapt_enabled}  "
          f"phi_thresh={cfg.trough_phi_thresh}")
    print(f"  [v10.0新增] 通道一 momentum_enabled={cfg.momentum_enabled}  "
          f"window={cfg.momentum_window}  "
          f"fast={cfg.momentum_decay_fast}  normal={cfg.momentum_decay_normal}")
    print(f"  [v10.0新增] 通道二 distortion_boost={cfg.distortion_boost_enabled}  "
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    momentum_arr = np.array(h['trough_momentum'])
    ...
    for pct in range(0, 101, 10):
        ...
        milestones.append({
            ...
            'momentum': float(momentum_arr[idx]), 'noise': float(noise_arr[idx]),
        })

    lines = []
    lines.append("# 涌积态宇宙模型 v10.0 — 运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    ...
    lines.append(f"| [v10] momentum_enabled | {cfg.momentum_enabled} |")
    lines.append(f"| [v10] momentum_window | {cfg.momentum_window} |")
    lines.append(f"| [v10] momentum_decay_fast/normal | {cfg.momentum_decay_fast} / {cfg.momentum_decay_normal} |")
    lines.append(f"| [v10] distortion_boost_enabled | {cfg.distortion_boost_enabled} |")
    lines.append(f"| [v10] distortion_boost_max | {cfg.distortion_boost_max} |")
    ...
    lines.append("| 进度 | tick | f_C2 | f_C1 | ΔΣ | Φ_net | C_var | 区块数 | Gini | Θ | 动量 | 噪声 | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i, m in enumerate(milestones):
        lines.append(
            ...
            f"{m['momentum']:+.4f} | {m['noise']:.3f} | {m['phase']} |"
        )
    lines.append("\n---\n")
    lines.append("## 六、全量时序数据（每帧）\n")
    lines.append("| tick | f_C2 | f_C1 | ΔΣ | Φ_net | C_var | 区块数 | Gini | Θ | 动量 | 噪声 | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i in range(len(ticks)):
        lines.append(
            ...
            f"{momentum_arr[i]:+.4f} | {noise_arr[i]:.3f} | {phases[i]} |"
        )
