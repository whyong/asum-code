"""
涌积态宇宙模型 v11.1 — 严格可复现与统计基准验证架构 (V11-1)
基于 v11.0（T-007 最终参数），引入统计可复现性与配置解耦机制：

  1. 冷却期决断与参数真实对齐：
     - 依据 T-002/T-003（历史最高演化态）的实证，默认禁用无效的 50000 帧冷却期（trough_ext_cooldown=0），
       彻底解决 v11.0 "封存配置与验证配置不一致" 的历史遗留问题；
     - 提供冷却期 A/B 对照能力，支持定量评估其拓扑影响。

  2. 严谨的可复现随机种子注入 (RNG Seeding Support)：
     - 支持通过配置项 / 环境变量 / CLI 参数注入独立随机数种子（rng_seed），
       既保证每次多样本独立试验的统计有效性，又确保单次实验具备 100% 绝对比特级可复现性。

  3. 完整的质量守恒修正（Deadband=0.001）与 A_instab 精确零均值算子。
...
import warnings, pathlib, os, sys, argparse
...
class Config:
    ...
    W, H   = 128, 128
    dt     = 0.05
    steps  = int(os.environ.get('V11_STEPS', 24000))

    # ── 随机种子（None=使用全局随机，整数=确定性可复现试验）─────────────
    rng_seed = int(os.environ['V11_SEED']) if 'V11_SEED' in os.environ else None
    ...
    # [v11.1] 冷却期配置（默认 0 即无冷却期，完全对齐 T-002/T-003 黄金参数）
    trough_ext_cooldown      = int(os.environ.get('V11_COOLDOWN', 0))
...
def run():
    ...
    print("=" * 65)
    print("涌积态宇宙模型 v11.1 启动")
    print(f"  网格: {cfg.W}×{cfg.H}   dt={cfg.dt}   最大帧数={cfg.steps}   RNG种子={cfg.rng_seed}")
    ...
    print(f"  [v11.0继承] 振荡计数器: thresh={cfg.osc_count_thresh}  window={cfg.osc_cvar_window}")
    print(f"  [v11.0继承] 双阈值扩展: ext_thresh={cfg.trough_fc2_thresh_ext}  "
          f"min_duration={cfg.trough_ext_min_duration}")
    print(f"  [v11.1决断] 扩展冷却期: cooldown={cfg.trough_ext_cooldown} ({'已禁用' if cfg.trough_ext_cooldown==0 else '已启用'})")
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    lines = []
    lines.append("# 涌积态宇宙模型 v11.1 — 运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    ...
    lines.append(f"| osc_count_thresh | {cfg.osc_count_thresh} |")
    lines.append(f"| osc_cvar_window / floor | {cfg.osc_cvar_window} / {cfg.osc_cvar_floor} |")
    lines.append(f"| trough_fc2_thresh_ext | {cfg.trough_fc2_thresh_ext} |")
    lines.append(f"| trough_phi_thresh_ext | {cfg.trough_phi_thresh_ext} |")
    lines.append(f"| trough_decay_rate_ext | {cfg.trough_decay_rate_ext} |")
    lines.append(f"| trough_ext_min_duration | {cfg.trough_ext_min_duration} |")
    lines.append(f"| trough_ext_cooldown | {cfg.trough_ext_cooldown} |")
