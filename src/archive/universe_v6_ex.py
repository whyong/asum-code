"""
涌积态宇宙模型 v6_ex — 广义重构版
基于 v6.0，将具体物理实现抽象为两个广义算子：
  - 有损算子 L：统一处理所有特征张量（状态浓度 + 变化率）
  - 失真算子 D：宏观因果场 + 拉普拉斯惯性通量 + 乘性噪声

与 v6.0 的映射关系：
  v6.0 的 I_stock_P/C1/C2（变率存量）= L(chi_rate_P/C1/C2)
  v6.0 的 S_C2（状态记忆）           = L(chi_state_C2)
  v6.0 的 Drive_base                 = I_norm + gamma_s*S_norm + kappa*∇²S_norm
  广义版的 I_macro                   = Σ w_i * L(χ_i)，统一累加所有特征张量的有损存量
...
class Config:
    ...
    steps  = 12000        # 默认帧数（24000 帧验证多代演化周期，日常运行用 12000）        # 默认帧数
    ...
    # ── [v6_ex] 广义算子参数 ──────────────────────────────────────────
    # 特征张量权重（统一的 I_macro 累加权重）
    # 变率特征群（原激变代谢）
    w_rate_P   = 0.1    # P 变率权重（恢复原值，启动期必需）
    w_rate_C1  = 0.7    # C1 变率权重（降低，避免 I_macro 量级过大）
    w_rate_C2  = 2.0    # C2 变率权重（降低，C2 积累后不过度主导 I_macro）
    # 状态特征群（原幽灵拓扑）
    w_state_C1 = 0.15   # C1 状态权重（降低一半，稳态期贡献适中）
    w_state_C2 = 0.25   # C2 状态权重（降低一半，对应 v6.0 gamma_s≈0.25）

    # 各特征张量的有损算子参数（遗忘率 lambda 和写入门槛 k_write）
    lam_rate_P   = 0.15    # P 变率遗忘率（对应 v6.0 loss_P）
    lam_rate_C1  = 0.04    # C1 变率遗忘率（对应 v6.0 loss_C1）
    lam_rate_C2  = 0.005   # C2 变率遗忘率（对应 v6.0 loss_C2）
    lam_state_C1 = 0.005   # C1 状态遗忘率（慢速追踪）
    lam_state_C2 = 0.001   # C2 状态遗忘率（对应 v6.0 lambda_s）

    kw_rate_P   = 0.001    # P 变率写入门槛
    kw_rate_C1  = 0.005    # C1 变率写入门槛
    kw_rate_C2  = 0.0005   # C2 变率写入门槛
    kw_state_C1 = 0.05     # C1 状态写入门槛（状态场直接写入，门槛较高）
    kw_state_C2 = 0.05     # C2 状态写入门槛

    k_norm_base = 0.002    # 统一归一化系数（对应 v6.0 k_norm）

    # 失真算子参数
    kappa_inertia = 0.3    # 拉普拉斯惯性通量强度（对应 v6.0 kappa_inertia）

    # 涌现度规（可选，v6.0 未启用）
    alpha_metric  = 0.0    # 空间弯曲强度（0=禁用，建议先保持 0）
...
class AdaptiveTuner:
    ...
    # 参数硬性边界（防止调参失控）
    # 注意：移除 gamma_s 和 kappa_inertia（这些参数不再被规则调整）
    BOUNDS = {
    ...
    def _check_rules(self, tick, phase, f_C2, f_C1, phi_net, c_var, delta_sig):
        ...
        if (phase == 'TURING_GROWTH'
            ...
            old = cfg.lam_rate_C2
            new = max(self.BOUNDS['lam_rate_C2'][0], old * 0.80)
            if new < old - 1e-6:
                self._apply('lam_rate_C2', old, new, tick,
                            f'图灵期衰退预警：Phi_net和C_var持续下降，降低lam_rate_C2增强记忆')
        ...
        if (self._chaotic_cnt >= 300
                ...
                and f_C2 < 0.20):
            old = cfg.lam_rate_C1
            new = max(self.BOUNDS['lam_rate_C1'][0], old * 0.80)
            if new < old - 1e-6:
                self._apply('lam_rate_C1', old, new, tick,
                            f'CHAOTIC_EDGE锁死{self._chaotic_cnt}帧，降低lam_rate_C1延长C1记忆')
                self._chaotic_cnt = 0
        ...
        if (phase == 'UNIFORM_GROWTH'
                ...
                and c_var < 0.001
                and f_C2 > 0.1
                and cfg.lam_rate_C1 < 0.03
                and (not self.log or tick - self.log[-1][0] >= 200)):
            old = cfg.lam_rate_C2
            new = min(self.BOUNDS['lam_rate_C2'][1], old * 1.10)
            if new > old + 1e-6:
                self._apply('lam_rate_C2', old, new, tick,
                            f'C_var长期过低({c_var:.5f})，UNIFORM_GROWTH已持续{tick}帧，'
                            f'提高lam_rate_C2恢复空间分化')

        # ── 规则6：lam_rate_C2 上调后仍崩溃 ─────────────────────────
        if (self._chaotic_cnt >= 200
                and phi_net == 0.0
                and cfg.lam_rate_C2 > 0.007
                and (not self.log or tick - self.log[-1][0] >= 200)):
            old = cfg.lam_rate_C2
            new = max(0.005, old * 0.90)
            if new < old - 1e-6:
                self._apply('lam_rate_C2', old, new, tick,
                            f'lam_rate_C2上调后仍CHAOTIC_EDGE锁死{self._chaotic_cnt}帧，'
                            f'回调lam_rate_C2防止记忆衰减过快')
        ...
        # 规则8（v6_ex）：已禁用
        # 原 v6.0 逻辑：图灵期退出后 C_var 持续低时提高 gamma_s 和 kappa_inertia
        # v6_ex 中 gamma_s/kappa_inertia 已被广义算子权重取代，不应被规则调整
...
class PhysicsLayer:
    def __init__(self, cfg: Config):
        ...
        # [v6_ex] 五个独立存量场（替代 v6.0 的 I_stock_P/C1/C2 + S_C2）
    ...
    def step(self):
        ...
        I_macro_rate_sum = np.zeros_like(P)  # 用于热力学代价计算

        # 特征矩阵配置：(chi, stock_attr, k_write, lam, weight, is_rate)
        features = [
            (chi_rate_P,   'stock_rate_P',   cfg.kw_rate_P,   cfg.lam_rate_P,   cfg.w_rate_P,   True),
            (chi_rate_C1,  'stock_rate_C1',  cfg.kw_rate_C1,  cfg.lam_rate_C1,  cfg.w_rate_C1,  True),
            (chi_rate_C2,  'stock_rate_C2',  cfg.kw_rate_C2,  cfg.lam_rate_C2,  cfg.w_rate_C2,  True),
            (chi_state_C1, 'stock_state_C1', cfg.kw_state_C1, cfg.lam_state_C1, cfg.w_state_C1, False),
            (chi_state_C2, 'stock_state_C2', cfg.kw_state_C2, cfg.lam_state_C2, cfg.w_state_C2, False),
        ]

        for chi, stock_name, k_w, lam, weight, is_rate in features:
            stock = getattr(self, stock_name)
            # Extract：概率门控
            p_write = chi / (chi + k_w)
            mask = (np.random.rand(*P.shape) < p_write).astype(np.float32)
            # Forget：指数遗忘
            stock[:] = stock * (1.0 - lam) + chi * mask * cfg.dt
            # 归一化并累加到宏观因果场
        ...
        # 2. 拉普拉斯惯性通量：只作用在状态特征群上（对应 v6.0 的 kappa * ∇²S_norm）
        #    变率特征群的拉普拉斯在 UNIFORM_GROWTH 期接近零，不提供有效向心引力
        ...
        self._I_norm_vis = I_macro / (I_macro.max() + 1e-9)  # 归一化供可视化

        # ── 步骤5：形态更新 ──────────────────────────────────────────
        # 涌现度规（alpha_metric=0 时退化为标准扩散）
        ...
        # 热力学磨损：激变剧烈时 C2 退化加速
        decay_C2_dynamic = cfg.decay_C2 + cfg.k_entropy * I_macro_rate_sum.mean()
...
class ObservationLayer:
    ...
    def observe(self, phys: PhysicsLayer, tick: int):
        ...
        sig_loss  = cfg.lam_rate_C2 * I_bar * P.size
...
def render(phys: PhysicsLayer, obs: ObservationLayer, tick: int):
    ...
    # 阶段色带：合并连续相同阶段，从 O(N) 次 axvspan 降至 O(相变次数) 次
    if len(ticks_plot) > 1:
...
def run():
    ...
    print(f"    遗忘率:   lam_rate_P={cfg.lam_rate_P}  lam_rate_C1={cfg.lam_rate_C1}  lam_rate_C2={cfg.lam_rate_C2}")
    print(f"              lam_state_C1={cfg.lam_state_C1}  lam_state_C2={cfg.lam_state_C2}")
    print(f"    k_norm_base={cfg.k_norm_base}  kappa_inertia={cfg.kappa_inertia}  alpha_metric={cfg.alpha_metric}")
    print(f"    k_entropy={cfg.k_entropy}")
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    lines = []
    lines.append(f"# 涌积态宇宙模型 v6_ex — 运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    ...
    lines.append(f"| lam_rate_P / C1 / C2 | {cfg.lam_rate_P} / {cfg.lam_rate_C1} / {cfg.lam_rate_C2} |")
    lines.append(f"| lam_state_C1 / C2 | {cfg.lam_state_C1} / {cfg.lam_state_C2} |")
    lines.append(f"| kw_rate_P / C1 / C2 | {cfg.kw_rate_P} / {cfg.kw_rate_C1} / {cfg.kw_rate_C2} |")
    lines.append(f"| kw_state_C1 / C2 | {cfg.kw_state_C1} / {cfg.kw_state_C2} |")
    lines.append(f"| k_norm_base | {cfg.k_norm_base} |")
