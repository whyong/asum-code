"""
涌积态宇宙模型 v12.2 — 全复数量子关系场架构 (Universe v12.2: Full Complex Relational Quantum Field)
基于 v12.1（纯关系矩阵场）与 v12.0（塔蒂尼量子层），实现全链条复数相位物理与干涉动力学：

═════════════════════════════════════════════════════════════════════════════
  核心物理跃迁：从“复数关系”到“全复数量子场 + 干涉与相位锁定”
═════════════════════════════════════════════════════════════════════════════

  1. P 场全复数波函数化（Complex Wavefunction Field）：
     - 概率波 P 升级为复数场 P_i = |P_i|·e^(iθ_i) ∈ ℂ^N (N=128)；
     - 振幅 |P_i| 对应微观概率幅，相位 θ_i 对应量子相干相位；
     - C1 (坍缩态) 与 C2 (退相干态) 保持实数标量，物理上对应退相干后的经典确定性记录。

  2. 真实波动干涉与相消湮灭（Emergent Interference & Annihilation）：
     - 守恒两通道输运 (-D_P·L̃@P + D_adv·Ã@P) 直接作用于复向量 P；
     - 具有反相 (Δθ ≈ π) 的节点间扩散流发生真实相消干涉（Destructive Interference）；
     - 干涉导致的局域模长 |P| 下降直接抑制宏观坍缩激发率，波动力学直接调控经典涌现。

  3. 塔蒂尼双重非线性效应（Dual Tartini Effect: Amplitude Pump + Self-Phase Modulation）：
     - 幅度通道：A_tartini = β·|P|² 驱动 P → C1 量子坍缩注能；
     - 相位通道：-i·γ·|P|²·P 引入 Gross-Pitaevskii 式非线性自相位调制 (SPM)，严格保持模长。

  4. 宏观量子相干序参量（Kuramoto Order Parameter）：
     - r(t)·e^(iΘ(t)) = (Σ P_i) / (Σ |P_i|)；
     - r ∈ [0, 1] 严格量化宇宙全网络的宏观相位同步度与相干凝聚度。
...
class AdaptiveTuner:
    ...
    def _check_rules(self, tick, phase, f_C2, f_C1, phi_net, c_var, delta_sig):
        ...
        if (phase in ('UNIFORM_GROWTH', 'TURING_GROWTH') and f_C1 > 0.15
...
# ─────────────────────────────────────────────
# 三、全复数量子关系场物理层 (Complex Quantum Physics Layer)
# ─────────────────────────────────────────────
...
class PhysicsLayer:
    def __init__(self, cfg: Config):
        ...
        # v12.2 全复数量子特有观测量
        self.kuramoto_r      = 1.0  # 全局 Kuramoto 序参量 (0~1)
        self.kuramoto_theta  = 0.0  # 全局均相角 (-π~π)
        self.phase_var       = 0.0  # 节点相角方差
    ...
    def step(self, tick: int = 0, c_var: float = 0.0, phase: str = 'CHAOTIC_EDGE'):
        ...
        # 复模长差分
        chi_rate_P  = np.abs(P - self.P_prev).astype(np.float32)
        ...
        # ── 步骤7：全复数量子演化与相干跃迁 ─────────────────────────
        # 1. 跃迁速率与严格模长非超额消耗限幅 (避免负幅翻相)
        A_P_raw = np.maximum(0, I_drive) * cfg.Base_Rate_1 * abs_P
        ...
        dP_amplitude_gain = (A_C1_to_P + A_C1_decay + A_C2_sink) * exp_i_theta
...
class ObservationLayer:
    def __init__(self, cfg: Config):
        ...
        self.history = {k: [] for k in [
            ...
            'kuramoto_r', 'kuramoto_theta', 'phase_var', # v12.2 新增
        ]}
        self._hd_buf = []
...
def render(phys: PhysicsLayer, obs: ObservationLayer, tick: int):
    ...
    ax2.legend(loc='upper right', fontsize=6, facecolor='#1a1a1a', labelcolor='white')
    ax2.set_xlim(-1.2, 1.2)
    ax2.set_ylim(-1.2, 1.2)
    ax2.tick_params(colors='white', labelsize=6)
    ax2.set_title('P 场复平面相图与相干矢量', color='white', fontsize=7, pad=3)
    ...
    ax4.scatter(node_idx, phases_P, c=abs_P, cmap='plasma', s=12)
    ax4.set_ylim(-np.pi * 1.1, np.pi * 1.1)
    ax4.axhline(0, color='gray', lw=0.5, linestyle=':')
    ax4.tick_params(colors='white', labelsize=6)
    ax4.set_xlabel('节点序号', color='white', fontsize=7)
    ax4.set_title(f'P 场相角分布 (Var={phys.phase_var:.3f})', color='#aaaaaa', fontsize=7, pad=3)
    ...
    ax5.set_xlabel('降序节点排列', color='white', fontsize=7)
    ...
    if _n > 1000:
        ...
        h_plot = {k: [h[k][i] for i in _idx] for k in
                  ['f_C2', 'f_C1', 'f_P', 'C_var', 'Phi_net', 'Theta_barrier',
                   'lpi', 'kuramoto_r', 'phase_flux', 'osc_count', 'dual_thresh_active', 'phase']}
    else:
        ticks_plot = ticks
        h_plot = h
    ...
    ax_ts.set_title('全复数关系场全局时序', color='#aaaaaa', fontsize=8, pad=4)
    ...
    ax_km.set_title('宏观量子相干序参量 r(t)', color='#aaaaaa', fontsize=8, pad=4)

    # 相位环流通量与振荡
    ax_ph = fig.add_subplot(gs[1, 4])
    ax_ph.set_facecolor('#1a1a1a')
    ax_ph.plot(ticks_plot, h_plot['phase_flux'], color='#1abc9c', lw=1.0, label='虚部环流通量')
    ax_ph.plot(ticks_plot, h_plot['osc_count'],  color='#e91e63', lw=1.0, label='振荡计数')
    ax_ph.legend(loc='upper left', fontsize=6, facecolor='#1a1a1a', labelcolor='white')
    ax_ph.tick_params(colors='white', labelsize=7)
    ax_ph.set_title('虚部环流与周波数', color='#aaaaaa', fontsize=8, pad=4)

    phase_now = h['phase'][-1] if h['phase'] else '—'
    color_now = PHASE_COLORS.get(phase_now, 'white')
    fig.suptitle(
        f'涌积态宇宙 v12.2（全复数量子场）   第 {tick:05d} 帧   '
        f'N=128   β={cfg.beta_tartini:.4f}   γ={cfg.gamma_spm:.4f}   '
        f'Kuramoto r={phys.kuramoto_r:.3f}   LPI={h["lpi"][-1]:.3f}   '
        f'阶段：{phase_now}',
...
def run():
    ...
    print("=" * 70)
    print("涌积态宇宙模型 v12.2 — 全复数量子关系场架构启动")
    print(f"  节点规模: N={cfg.N_NODES} (全复数 P 场)   dt={cfg.dt}   最大帧数={cfg.steps}")
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    lines = []
    lines.append("# 涌积态宇宙模型 v12.2 — 全复数量子关系场运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    ...
    lines.append("---\n")
    lines.append("## 一、核心量子关系参数\n")
    lines.append("| 参数 | 值 | 说明 |")
    lines.append("|---|---|---|")
    lines.append(f"| 节点数 N | {cfg.N_NODES} | 全复数波函数场 P |")
    ...
    lines.append("| 进度 | tick | f_P | f_C1 | f_C2 | C_var | LPI | Kuramoto r | Gini | Theta | 振荡 | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i, m in enumerate(milestones):
        lines.append(
            f"| {i*10}% | {m['tick']} | {m['f_P']:.4f} | {m['f_C1']:.4f} | {m['f_C2']:.4f} | "
            f"{m['C_var']:.5f} | {m['lpi']:.4f} | {m['km_r']:.4f} | {m['gini']:.4f} | {m['theta']:.4f} | "
            f"{m['osc']} | {m['phase']} |"
        )
    lines.append("\n---\n")
    lines.append("## 六、全量时序数据（每帧）\n")
    lines.append("| tick | f_P | f_C1 | f_C2 | C_var | LPI | Kuramoto r | Gini | Theta | 振荡 | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|")
    for i in range(len(ticks)):
        lines.append(
            f"| {int(ticks[i])} | {f_p[i]:.4f} | {f_c1[i]:.4f} | {f_c2[i]:.4f} | "
            f"{c_var[i]:.5f} | {lpi_arr[i]:.4f} | {km_r[i]:.4f} | {gini_arr[i]:.4f} | {theta_arr[i]:.4f} | "
