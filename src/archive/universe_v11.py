"""
涌积态宇宙模型 v11.0 — 振荡计数器 + 双阈值扩展架构
基于 v10.0（T-011 最终参数），针对亚低谷振荡瓶颈的根本性突破：

  核心问题（v9.0/v10.0 共同瓶颈）：
    系统在长程运行（tick > 34000）后进入 f_C2≈0.22~0.28 的亚低谷吸引子，
    trough_adapt 的触发条件（f_C2 < 0.20）无法覆盖此区间，
    C_var 停滞在 0.06~0.08，无法自然突破到 v8.0 的高位稳定态（C_var≈0.05+）。

  根本原因分析：
    亚低谷吸引子的本质是 C2 的生成通量（Phi_net≈25~35）与衰减通量
    （decay_C2×C2）达到了动态平衡，导致 f_C2 稳定在 0.22~0.28。
    这个平衡点比 trough_adapt 的触发阈值（f_C2 < 0.20）高出约 0.03，
    是结构性的——系统在此吸引子内部的动力学不会自然将 f_C2 推低到 0.20。

  v11.0 的解决方案：振荡计数器 + 双阈值扩展（Oscillation Counter + Dual-Threshold）

  机制一：振荡计数器（Oscillation Counter）
    统计系统在 TURING_GROWTH 期内 C_var 的完整振荡次数。
    C_var 从高于滑动均值降到低于均值再升回来算一次完整振荡。
    当振荡次数超过阈值（osc_count_thresh=3）时，判定系统陷入亚稳态振荡。
    铁律对齐：振荡计数器是有损算子的"时间沉淀"——用历史振荡次数作为
    系统陷入亚稳态的宏观拓扑趋势指标，而非瞬时值。

  机制二：双阈值扩展（Dual-Threshold Extension）
    在振荡计数器确认系统陷入亚稳态后，将 trough_adapt 的触发阈值
    从 trough_fc2_thresh=0.20 扩展到 trough_fc2_thresh_ext=0.28，
    覆盖亚低谷振荡区间（f_C2=0.22~0.28）。
    同时要求 C_var > cvar_floor=0.03（排除启动期 C_var≈0 的情况）。
    扩展触发时使用更快的衰减率（trough_decay_rate_ext=0.9995），
    加速势垒降低，帮助系统更快跨越亚低谷吸引子。

  铁律对齐：
    振荡计数器（有损沉淀）= 提取"系统陷入亚稳态"的宏观历史趋势
    双阈值扩展（失真涨落）= 在确认亚稳态后增强对称性破缺，探索相空间
    两者共同作用 = 秩序（有损沉淀）与混沌（失真涨落）对抗后的残差加速

  与 v10.0 的关键区别：
    v10.0 的通道一（C_var 停滞检测）在图灵期初期就误触发，因为它无法
    区分"图灵期正常的 C_var 峰值后下降"和"长期亚稳态振荡"。
    v11.0 的振荡计数器通过计数完整振荡次数（而非检测瞬时趋势），
    天然地要求系统已经经历了多次振荡才触发，避免了初期误触发。
...
class Config:
    ...
    W, H   = 128, 128
    dt     = 0.05
    steps  = 24000   # 日常运行默认值（中程用 200000，长程用 500000）
    ...
    # ── [v8.0 继承] 涌现度规 ─────────────────────────────────────────
    alpha_metric    = 0.3
    alpha_metric_C1 = 0.0   # 禁用（v9.0 T-003 确认）
    ...
    # ── [v9.0 继承] 低谷期自适应势垒（基础触发，继承不变）────────────
    trough_adapt_enabled  = True
    trough_fc2_thresh     = 0.20   # 基础阈值（v9.0 最优值）
    ...
    # ── [v11.0 新增] 机制一：振荡计数器 ──────────────────────────────
    # 统计 TURING_GROWTH 期内 C_var 的完整振荡次数
    # C_var 从高于滑动均值降到低于均值再升回来算一次完整振荡
    # 当振荡次数 >= osc_count_thresh 时，判定系统陷入亚稳态振荡
    osc_counter_enabled   = True
    osc_count_thresh      = 3      # 需要经历 3 次完整振荡才触发扩展
    osc_cvar_window       = 500    # 计算 C_var 滑动均值的窗口（帧数）
    osc_cvar_floor        = 0.03   # C_var 低于此值时不计入振荡（排除启动期）

    # ── [v11.0 新增] 机制二：双阈值扩展 ──────────────────────────────
    # 在振荡计数器确认亚稳态后，扩展 trough_adapt 的触发范围
    # 覆盖亚低谷振荡区间（f_C2=0.22~0.28）
    dual_thresh_enabled      = True
    trough_fc2_thresh_ext    = 0.28   # T-006: 恢复（原0.45过高），只覆盖亚低谷区
    trough_phi_thresh_ext    = 20.0   # 扩展时的 Phi_net 阈值
    trough_decay_rate_ext    = 0.9999 # 扩展时的衰减率（与基础相同，不加速）
    # 扩展触发的额外保护：
    # 1. C_var 必须 > osc_cvar_floor（排除启动期）
    # 2. phase == TURING_GROWTH（已在外层保证）
    # 3. f_C2 持续低于 ext_thresh 超过 trough_ext_min_duration 帧才触发
    trough_ext_min_duration  = 2000   # 持续低谷帧数阈值
    # [v11.0 T-006 新增] 扩展阈值冷却期：触发一次后等待 N 帧才能再次触发
    # 防止连续触发导致 Theta 过度降低
    trough_ext_cooldown      = 50000  # 冷却期（帧数）：约 2500 秒模拟时间
...
class AdaptiveTuner:
    ...
    def _check_rules(self, tick, phase, f_C2, f_C1, phi_net, c_var, delta_sig):
        ...
        # 规则2：C2 积累过慢（弱化：仅在 Phi_net 极低时触发）
...
class PhysicsLayer:
    def __init__(self, cfg: Config):
        ...
        # [v11.0 新增] 振荡计数器状态
        self._cvar_window_buf  = []    # 滑动窗口缓冲（用于计算 C_var 均值）
        self._cvar_above_mean  = False # 当前 C_var 是否高于滑动均值
        self._osc_count        = 0     # 已完成的完整振荡次数
        self._osc_confirmed    = False # 是否已确认陷入亚稳态振荡
        self.osc_count_vis     = 0     # 供观测层读取
        self.dual_thresh_active = False # 当前是否使用扩展阈值
        self._ext_below_count  = 0     # f_C2 持续低于 ext_thresh 的帧数计数
        self._ext_cooldown_remaining = 0  # 扩展阈值冷却期剩余帧数
    ...
    def step(self, tick: int = 0, c_var: float = 0.0, phase: str = 'CHAOTIC_EDGE'):
        ...
        # ── 步骤4：D 失真算子（含通道二失真增强）────────────────────
        # 通道二：trough_active 时增强失真（继承 v10.0）
        ...
        # 记录更新前总质量（PDE 守恒，理论上更新后总质量应等于此值）
        ...
        # ── 截断后守恒修正 ───────────────────────────────────────────
        # float32 精度误差导致 PDE 更新后总质量略有变化（每帧约 ±1e-3）
        # 只在质量变化超过噪声阈值（0.001）时才修正，避免对 float32 噪声过度修正
        # 阈值选取依据：float32 每步噪声约 ±1e-3，真实截断误差约 ±2e-3
        M_after = float(self.P.sum() + self.C1.sum() + self.C2.sum())
        mass_change = M_after - M_before
        if abs(mass_change) > 0.001 and M_after > 1e-9:
            # 质量变化超过阈值（真实截断误差），等比例修正
        ...
        if phase == 'TURING_GROWTH':
            ...
            if cfg.osc_counter_enabled and c_var > cfg.osc_cvar_floor:
                ...
                if len(self._cvar_window_buf) >= cfg.osc_cvar_window // 2:
                    ...
                    # 检测完整振荡：从高于均值 → 低于均值 → 高于均值
                    # 当前帧从低于均值变为高于均值，且之前曾低于均值
            ...
            # 扩展触发条件（v11.0 新增）：振荡计数器确认亚稳态后才激活
            # 额外保护：f_C2 必须持续低于 ext_thresh 超过 min_duration 帧
            # 且冷却期已结束（防止连续触发导致 Theta 过度降低）
            ...
            if (cfg.dual_thresh_enabled
                ...
                self._ext_below_count = 0  # 重置计数（f_C2 回升、冷却中或条件不满足）
            ...
            # 不在 TURING_GROWTH 期，重置振荡计数器（但保留 _osc_confirmed 状态）
            # 注意：_osc_confirmed 不重置，一旦确认亚稳态就永久有效
            # 注意：冷却期计数器不重置（跨越 UNIFORM_GROWTH 期继续冷却）
            self._cvar_window_buf.clear()
            self._cvar_above_mean = False
            self._ext_below_count = 0   # 离开图灵期时重置持续计数
            self._trough_active = False
            self.dual_thresh_active = False


# ─────────────────────────────────────────────
# 四、观测层（继承 v10.0，新增 v11.0 指标）
# ─────────────────────────────────────────────
...
class ObservationLayer:
    def __init__(self, cfg: Config):
        ...
        self.history = {k: [] for k in [
            ...
            'k_noise_eff',       # [v10.0]
            'osc_count',         # [v11.0 新增] 振荡计数
            'dual_thresh_active', # [v11.0 新增] 扩展阈值是否激活
        ]}
        self._hd_buf = []
...
def render(phys: PhysicsLayer, obs: ObservationLayer, tick: int):
    ...
    fields = [
        ...
        (phys.metric_vis,  'plasma',   None,            '[v8] 涌现度规\n亮=扩散正常  暗=时空弯曲锁定'),
    ...
    for key, color, lbl in [
        ...
        ('k_noise_eff',      '#1abc9c', '[v10] 有效噪声强度'),
        ('dual_thresh_active','#e91e63', '[v11] 扩展阈值激活'),
    ]:
    ...
    ax_cl.plot(ticks_plot, h_plot['osc_count'],  color='#e91e63', lw=1.0, label='[v11]振荡计数')
    ...
    ax_cl.set_title('空间结构统计 + 振荡计数', color='#aaaaaa', fontsize=8, pad=4)
    ...
    osc_now   = h['osc_count'][-1] if h['osc_count'] else 0
    dual_now  = h['dual_thresh_active'][-1] if h['dual_thresh_active'] else 0
    fig.suptitle(
        f'涌积态宇宙 v11.0   第 {tick:05d} 帧   '
        f'C2占比={h["f_C2"][-1]:.3f}   Θ={theta_now:.4f}   '
        f'振荡={osc_now}次   扩展阈值={"ON" if dual_now else "off"}   '
    ...
    print(f'  [render] tick={tick:05d}  phase={phase_now}  '
          ...
          f'osc={osc_now}  dual={"ON" if dual_now else "off"}')
...
def run():
    ...
    print("=" * 65)
    print("涌积态宇宙模型 v11.0 启动")
    print(f"  网格: {cfg.W}×{cfg.H}   dt={cfg.dt}   最大帧数={cfg.steps}")
    print(f"  [v8.0继承] alpha_metric={cfg.alpha_metric}  seed_freq={cfg.seed_freq}")
    print(f"  [v9.0继承] trough_adapt={cfg.trough_adapt_enabled}  "
          f"fc2_thresh={cfg.trough_fc2_thresh}  phi_thresh={cfg.trough_phi_thresh}")
    print(f"  [v10.0继承] distortion_boost={cfg.distortion_boost_enabled}  "
          f"max_boost={cfg.distortion_boost_max}")
    print(f"  [v11.0新增] 振荡计数器: osc_count_thresh={cfg.osc_count_thresh}  "
          f"cvar_window={cfg.osc_cvar_window}  cvar_floor={cfg.osc_cvar_floor}")
    print(f"  [v11.0新增] 双阈值扩展: fc2_thresh_ext={cfg.trough_fc2_thresh_ext}  "
          f"phi_thresh_ext={cfg.trough_phi_thresh_ext}  "
          f"decay_ext={cfg.trough_decay_rate_ext}  "
          f"min_duration={cfg.trough_ext_min_duration}  "
          f"cooldown={cfg.trough_ext_cooldown}")
    ...
    for tick in range(cfg.steps):
        ...
        if tick % 500 == 0 and tick > 0:
            ...
            print(f'  [progress] {pct:5.1f}%  tick={tick:06d}  '
                  ...
                  f'osc={h["osc_count"][-1]}  '
                  f'dual={"ON" if h["dual_thresh_active"][-1] else "off"}  '
                  f'noise={h["k_noise_eff"][-1]:.3f}')
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    noise_arr = np.array(h['k_noise_eff'])
    ...
    for pct in range(0, 101, 10):
        ...
        milestones.append({
            ...
            'osc': int(osc_arr[idx]), 'dual': int(dual_arr[idx]),
            'noise': float(noise_arr[idx]),
        })

    lines = []
    lines.append("# 涌积态宇宙模型 v11.0 — 运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    ...
    lines.append(f"| distortion_boost_max | {cfg.distortion_boost_max} |")
    lines.append(f"| [v11] osc_count_thresh | {cfg.osc_count_thresh} |")
    lines.append(f"| [v11] osc_cvar_window / floor | {cfg.osc_cvar_window} / {cfg.osc_cvar_floor} |")
    lines.append(f"| [v11] trough_fc2_thresh_ext | {cfg.trough_fc2_thresh_ext} |")
    lines.append(f"| [v11] trough_phi_thresh_ext | {cfg.trough_phi_thresh_ext} |")
    lines.append(f"| [v11] trough_decay_rate_ext | {cfg.trough_decay_rate_ext} |")
    lines.append(f"| [v11] trough_ext_min_duration / cooldown | {cfg.trough_ext_min_duration} / {cfg.trough_ext_cooldown} |")
    ...
    lines.append("| 进度 | tick | f_C2 | f_C1 | ΔΣ | Φ_net | C_var | 区块数 | Gini | Θ | 振荡 | 扩展 | 噪声 | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i, m in enumerate(milestones):
        lines.append(
            f"| {i*10}% | {m['tick']} | {m['f_C2']:.4f} | {m['f_C1']:.4f} | "
            f"{m['delta_sig']:.4e} | {m['phi_net']:.4e} | {m['C_var']:.5f} | "
            f"{m['n_cl']} | {m['gini']:.4f} | {m['theta']:.4f} | "
            f"{m['osc']} | {'ON' if m['dual'] else 'off'} | {m['noise']:.3f} | {m['phase']} |"
        )
    lines.append("\n---\n")
    lines.append("## 六、全量时序数据（每帧）\n")
    lines.append("| tick | f_C2 | f_C1 | ΔΣ | Φ_net | C_var | 区块数 | Gini | Θ | 振荡 | 扩展 | 噪声 | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i in range(len(ticks)):
        lines.append(
            f"| {int(ticks[i])} | {f_c2[i]:.4f} | {f_c1[i]:.4f} | "
            f"{delta_sig[i]:.4e} | {phi_net[i]:.4e} | {c_var[i]:.5f} | "
            f"{int(n_cl[i])} | {gini_arr[i]:.4f} | {theta_arr[i]:.4f} | "
            f"{int(osc_arr[i])} | {'ON' if dual_arr[i] else 'off'} | {noise_arr[i]:.3f} | {phases[i]} |"
        )
