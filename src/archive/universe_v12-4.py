"""
涌积态宇宙模型 v12.4 — 关系演化宇宙架构 (Universe v12.4: Evolving Relational Universe / Network Learning)
基于 v12.3（相干驱动与量子测量），实现权重矩阵自身随波函数相干性演化的动力学闭环：

═════════════════════════════════════════════════════════════════════════════
  核心物理跃迁：宇宙通过复数 Hebbian 机制自组织演化其关系拓扑
═════════════════════════════════════════════════════════════════════════════

  1. 复数 Hebbian 关系可塑性（Complex Hebbian Relational Plasticity）：
     - 节点波函数外积 H = P P† ∈ ℂ^{N×N}, H_ij = P_i · conj(P_j) = |P_i||P_j|·e^(i(θ_i - θ_j))；
     - 实部 Re(H_ij) = |P_i||P_j|·cos(θ_i - θ_j) 具有天然严格对称性：
       * 同相节点 (cos > 0) 强化对称扩散键 K_sym；
       * 反相节点 (cos < 0) 弱化扩散键；
     - 虚部 Im(H_ij) = |P_i||P_j|·sin(θ_i - θ_j) 具有天然严格反对称性：
       * 正交相位差诱发定向环流通量 A_antisym；
     - 赫布学习构造性地恒等保持守恒分解定理！

  2. 拓扑演化微分方程与衰减稳态：
     - dK_sym/dt = η_H · (Re(P P†) ⊙ Mask_bond - ρ · K_sym)
     - dA_antisym/dt = η_H · (Im(P P†) ⊙ Mask_bond - ρ · A_antisym)
     - 动态度规 M_ij 继续在演化后的关系骨架上进行快调制。

  3. 谱半径重归一卫兵（Spectral Radius Stabilization Guard）：
     - 每 adapt_interval 周期性核验 λ_max(L̃) ≤ 8.0；
     - 确保显式欧拉时步 dt=0.05 的数值稳定性永不破缺。

  4. 拓扑涌现观测量（Evolving Topology Observables）：
     - 关系网络平均强度 (Mean Relational Strength): mean_K；
     - 节点度强度基尼系数 (Degree Strength Gini): degree_gini；
     - 赫布可塑性通量 (Plasticity Flux): plasticity_flux；
     - Kuramoto 全局序参量 r(t)、相位保真度 F(t)。
...
class Config:
    ...
    eta_hebbian      = float(os.environ.get('V12_ETA_H', 0.001))  # 赫布学习率
    rho_hebbian      = float(os.environ.get('V12_RHO', 0.01))     # 关系衰减率
...
# ─────────────────────────────────────────────
# 三、关系演化宇宙物理层 (Evolving Relational Physics Layer)
# ─────────────────────────────────────────────
...
class PhysicsLayer:
    def __init__(self, cfg: Config):
        ...
        for i in range(N):
            ...
            self.bond_mask[top_idx, i] = True  # 对称拓扑骨架
    ...
    def step(self, tick: int = 0, c_var: float = 0.0, phase: str = 'CHAOTIC_EDGE'):
        ...
        # ── 步骤6：复数 Hebbian 关系拓扑演化（核心 v12.4）───────────
        if cfg.eta_hebbian > 0:
            # 波函数外积 H_ij = P_i · conj(P_j)
            H_outer = P[:, None] * np.conj(P[None, :]) # (N, N) 复杂外积
...
class ObservationLayer:
    def __init__(self, cfg: Config):
        ...
        self.history = {k: [] for k in [
            ...
            'plasticity_flux', 'degree_gini', # v12.4 新增
        ]}
        self._hd_buf = []
...
def render(phys: PhysicsLayer, obs: ObservationLayer, tick: int):
    ...
    ax1.set_title(f'演化关系度规 K_eff (mean={phys.mean_eff_coupling:.3f})\n(复数 Hebbian 自组织塑形)', color='white', fontsize=7, pad=3)
    ax1.axis('off')
    ...
    ax4.set_title(f'节点关系度谱 (Gini={phys.degree_gini:.3f})', color='#aaaaaa', fontsize=7, pad=3)
    ...
    if _n > 1000:
        ...
        h_plot = {k: [h[k][i] for i in _idx] for k in
                  ['f_C2', 'f_C1', 'f_P', 'C_var', 'Phi_net', 'Theta_barrier',
                   'lpi', 'kuramoto_r', 'mean_eff_coupling', 'degree_gini', 'plasticity_flux', 'phase']}
    else:
        ticks_plot = ticks
        h_plot = h
    ...
    ax_ts.set_title('关系演化宇宙全局时序', color='#aaaaaa', fontsize=8, pad=4)

    # 关系拓扑演化时序
    ax_net = fig.add_subplot(gs[1, 3])
    ax_net.set_facecolor('#1a1a1a')
    ax_net.plot(ticks_plot, h_plot['mean_eff_coupling'], color='#e67e22', lw=1.0, label='平均关系强度')
    ax_net.plot(ticks_plot, h_plot['degree_gini'],        color='#f39c12', lw=1.0, label='关系基尼系数')
    ax_net.legend(loc='upper left', fontsize=6, facecolor='#1a1a1a', labelcolor='white')
    ax_net.tick_params(colors='white', labelsize=7)
    ax_net.set_xlabel('tick', color='white', fontsize=8)
    ax_net.set_title('关系拓扑自组织演化', color='#aaaaaa', fontsize=8, pad=4)

    # Kuramoto 序参量与赫布通量
    ax_km = fig.add_subplot(gs[1, 4])
    ax_km.set_facecolor('#1a1a1a')
    ax_km.plot(ticks_plot, h_plot['kuramoto_r'],      color='#00ffff', lw=1.0, label='Kuramoto r')
    ax_km.plot(ticks_plot, h_plot['plasticity_flux'], color='#e91e63', lw=1.0, label='赫布可塑通量')
    ax_km.legend(loc='upper left', fontsize=6, facecolor='#1a1a1a', labelcolor='white')
    ax_km.tick_params(colors='white', labelsize=7)
    ax_km.set_title('相干度与拓扑学习速率', color='#aaaaaa', fontsize=8, pad=4)

    phase_now = h['phase'][-1] if h['phase'] else '—'
    color_now = PHASE_COLORS.get(phase_now, 'white')
    fig.suptitle(
        f'涌积态宇宙 v12.4（关系演化宇宙）   第 {tick:05d} 帧   '
        f'N=128   η_H={cfg.eta_hebbian}   Kuramoto r={phys.kuramoto_r:.3f}   '
        f'度Gini={phys.degree_gini:.3f}   阶段：{phase_now}',
...
def run():
    ...
    print("=" * 70)
    print("涌积态宇宙模型 v12.4 — 关系演化宇宙架构启动")
    print(f"  节点规模: N={cfg.N_NODES} (拓扑自组织+复数赫布学习)   dt={cfg.dt}   最大帧数={cfg.steps}")
    ...
    print(f"  守恒分解: 对称扩散 (K_sym) + 反对称相位流 (A_antisym) (代数严格守恒)")
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    for pct in range(0, 101, 10):
        ...
        milestones.append({
            ...
            'res': float(res_r[idx]), 'theta': float(theta_arr[idx]), 'phase': phases[idx],
        })

    lines = []
    lines.append("# 涌积态宇宙模型 v12.4 — 关系演化宇宙运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    lines.append(f"**输出目录**：`{OUTPUT_DIR}`  ")
    lines.append(f"**RNG 种子**：`{cfg.rng_seed}`  ")
    lines.append(f"**赫布学习率 η_H**：`{cfg.eta_hebbian}`  ")
    lines.append(f"**关系衰减率 ρ**：`{cfg.rho_hebbian}`  ")
    lines.append(f"**塔蒂尼耦合系数 β**：`{cfg.beta_tartini}`  ")
    lines.append(f"**自相位调制系数 γ**：`{cfg.gamma_spm}`\n")
    lines.append("---\n")
    lines.append("## 一、核心关系演化参数\n")
    lines.append("| 参数 | 值 | 说明 |")
    lines.append("|---|---|---|")
    lines.append(f"| 节点数 N | {cfg.N_NODES} | 复数 Hebbian 自组织拓扑网络 |")
    ...
    lines.append("| 进度 | tick | f_P | f_C1 | f_C2 | C_var | LPI | Kuramoto r | <K> | 度Gini | 共振率 | Theta | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i, m in enumerate(milestones):
        lines.append(
            f"| {i*10}% | {m['tick']} | {m['f_P']:.4f} | {m['f_C1']:.4f} | {m['f_C2']:.4f} | "
            f"{m['C_var']:.5f} | {m['lpi']:.4f} | {m['km_r']:.4f} | {m['mean_k']:.4f} | {m['deg_g']:.4f} | "
            f"{m['res']:.4f} | {m['theta']:.4f} | {m['phase']} |"
        )
    lines.append("\n---\n")
    lines.append("## 六、全量时序数据（每帧）\n")
    lines.append("| tick | f_P | f_C1 | f_C2 | C_var | LPI | Kuramoto r | <K> | 度Gini | 共振率 | Theta | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i in range(len(ticks)):
        lines.append(
            f"| {int(ticks[i])} | {f_p[i]:.4f} | {f_c1[i]:.4f} | {f_c2[i]:.4f} | "
            f"{c_var[i]:.5f} | {lpi_arr[i]:.4f} | {km_r[i]:.4f} | {mean_k[i]:.4f} | {deg_g[i]:.4f} | "
