"""
涌积态宇宙模型 v12.5 — 全复数终态总装架构 (Universe v12.5: Grand Full-Complex Final Assembly)
聚合 v12.1 ~ v12.4 全系列复数物理与动力学特性，集成非厄米 PT 对称谱分析与六代大一统观测体系。
...
═════════════════════════════════════════════════════════════════════════════
  ...
  1. 关系矩阵本体论（v12.1）：
     - 守恒分解定理：实部对称扩散 (K_sym) + 虚部反对称相位流 (A_antisym)。
  2. 全复数量子场论（v12.2）：
     - 节点波函数 P ∈ ℂ^{128}，保模长酉算子自相位调制 SPM：exp(-i γ |P|² dt)。
     - Kuramoto 宏观相干序参量 r(t) 与局域相位干涉。
  3. 相干驱动与量子测量（v12.3）：
     - 复数五轨因果存量场，复圆高斯 Langevin 噪声，复驱动场 I_drive ∈ ℂ^{128}。
     - 相位共振坍缩跃迁：s_P = Re(I_drive · exp(-i θ_P))。
  4. 关系演化与网络可塑性（v12.4）：
     - 复数 Hebbian 学习闭环：ΔW = η_H · (P P† - ρ W) dt。
     - 谱半径重归一卫兵：λ_max(L̃) ≤ 8.0 稳定守门。
  5. 终态非厄米 PT 对称谱分析与大一统观测量（v12.5 终极新增）：
     - 构造非厄米有效哈密顿量/传播子 ℋ_eff；
     - 实时求解复特征谱 (Re(λ), Im(λ))，计算 PT 破缺比例与奇异点 (EP) 距离；
     - 实验性开关 `V12_COMPLEX_CLASSICAL`（默认 0，支持全三轨复数扩展研究）。
...
class Config:
    ...
    trough_ext_cooldown      = 0

    # ── 塔蒂尼量子非线性耦合 ─────────────────────────────────────────
    beta_tartini     = float(os.environ.get('V12_BETA', 0.0))
    gamma_spm        = float(os.environ.get('V12_GAMMA', 0.05))
    compute_lpi      = True

    # ── v12.4 复数 Hebbian 关系演化参数 ──────────────────────────────
    eta_hebbian      = float(os.environ.get('V12_ETA_H', 0.001))
    rho_hebbian      = float(os.environ.get('V12_RHO', 0.01))

    # ── v12.5 非厄米谱分析与经典复数实验开关 ─────────────────────────
    complex_classical = int(os.environ.get('V12_COMPLEX_CLASSICAL', 0)) == 1
    pt_analysis_interval = 100
...
# ─────────────────────────────────────────────
# 三、全复数终态物理层 (Grand Full-Complex Physics Layer)
# ─────────────────────────────────────────────
...
class PhysicsLayer:
    def __init__(self, cfg: Config):
        ...
        if cfg.complex_classical:
            self.C1 = np.zeros(N, np.complex64)
            self.C2 = np.zeros(N, np.complex64)
        else:
        ...
        k_bonds = 6
        abs_W = np.abs(W_raw)
        self.bond_mask = np.zeros((N, N), dtype=bool)
        W_sparse = np.zeros_like(W_raw)
        for i in range(N):
            top_idx = np.argsort(abs_W[i])[-k_bonds:]
            W_sparse[i, top_idx] = W_raw[i, top_idx]
            self.bond_mask[i, top_idx] = True
            self.bond_mask[top_idx, i] = True
        ...
        # 复数五轨存量场
        self.stock_rate_P   = np.zeros(N, np.complex64)
        ...
        self.I_drive         = np.zeros(N, np.complex64)
        ...
        self.phase_var       = 0.0
        self.res_ratio       = 1.0
        self.phase_fidelity  = 1.0
        self.plasticity_flux = 0.0
        self.degree_gini     = 0.0

        # v12.5 非厄米 PT 谱观测量
        self.pt_broken_ratio = 0.0
        self.pt_spectral_gap = 1.0
        self.hamiltonian_eigvals = np.zeros(N, np.complex64)
    ...
    def step(self, tick: int = 0, c_var: float = 0.0, phase: str = 'CHAOTIC_EDGE'):
        ...
        abs_C1 = np.abs(C1).astype(np.float32) if cfg.complex_classical else C1
        abs_C2 = np.abs(C2).astype(np.float32) if cfg.complex_classical else C2
        ...
        abs_C1_prev = np.abs(self.C1_prev).astype(np.float32) if cfg.complex_classical else self.C1_prev
        abs_C2_prev = np.abs(self.C2_prev).astype(np.float32) if cfg.complex_classical else self.C2_prev

        chi_rate_C1 = np.abs(abs_C1 - abs_C1_prev)
        chi_rate_C2 = np.abs(abs_C2 - abs_C2_prev)
        chi_state_C1 = abs_C1
        chi_state_C2 = abs_C2

        # ── 步骤2：H_sink 局部邻域方差检测 ──────────────────────────
        C2_lmean = self.R @ abs_C2
        C2_lm2   = self.R @ (abs_C2 ** 2)
        ...
        A_C2_sink = cfg.gamma_sink * sink_mask * abs_C2
        ...
        if cfg.distortion_boost_enabled and self._trough_active:
            M_total = abs_P.sum() + abs_C1.sum() + abs_C2.sum()
            f_C2_now = abs_C2.sum() / (M_total + 1e-9)
        ...
        # ── 步骤6：复数 Hebbian 关系拓扑演化 ─────────────────────────
        if cfg.eta_hebbian > 0:
            H_outer = P[:, None] * np.conj(P[None, :])
            H_real = np.real(H_outer).astype(np.float32)
            H_imag = np.imag(H_outer).astype(np.float32)
            
            dH_sym  = np.where(self.bond_mask, H_real, 0.0)
            dH_anti = np.where(self.bond_mask, H_imag, 0.0)
            
            dK_dt = cfg.eta_hebbian * (dH_sym  - cfg.rho_hebbian * self.K_sym)
            dA_dt = cfg.eta_hebbian * (dH_anti - cfg.rho_hebbian * self.A_antisym)
            
            K_new = self.K_sym + dK_dt * cfg.dt
            A_new = self.A_antisym + dA_dt * cfg.dt
            
            K_new = (K_new + K_new.T) / 2.0
            A_new = (A_new - A_new.T) / 2.0
            np.fill_diagonal(K_new, 0.0)
            np.fill_diagonal(A_new, 0.0)
            
            self.plasticity_flux = float(np.abs(dK_dt).mean() + np.abs(dA_dt).mean())
            self.K_sym = K_new
            self.A_antisym = A_new

            if tick % cfg.adapt_interval == 0:
                deg_K = self.K_sym.sum(axis=1)
                L_temp = np.diag(deg_K) - self.K_sym
                try:
                    eigvals, _ = np.linalg.eigh(L_temp)
                    max_lam = float(np.max(np.abs(eigvals)))
                    if max_lam > 8.0:
                        scale = 8.0 / (max_lam + 1e-9)
                        self.K_sym *= scale
                    norm_A = float(np.linalg.norm(self.A_antisym, 2))
                    if norm_A > 4.0:
                        self.A_antisym *= (4.0 / (norm_A + 1e-9))
                except Exception:
                    pass
        ...
        node_degrees = np.abs(K_eff).sum(axis=1)
        self.degree_gini = float(gini(node_degrees))
        
        L_eff = np.diag(K_eff.sum(axis=1)) - K_eff

        # ── 步骤7：相位共振坍缩与相干跃迁 ───────────────────────────
        s_P = np.real(I_drive * np.conj(exp_i_theta)).astype(np.float32)
        self.res_ratio = float(np.mean(s_P > 0.0))
        ...
        A_C1_to_P  = np.abs(np.minimum(0.0, s_P)) * cfg.Base_Rate_1 * abs_C1
        A_C1_to_C2 = np.maximum(0.0, s_C - cfg.Theta_barrier) * cfg.Base_Rate_2 * abs_C1
        A_C2_to_C1 = np.abs(np.minimum(0.0, s_C)) * cfg.Base_Rate_dec * abs_C2

        A_C1_decay = cfg.decay_C1 * abs_C1
        decay_C2_dyn = cfg.decay_C2 + cfg.k_entropy * I_macro_rate_sum.mean()
        A_C2_decay   = decay_C2_dyn * abs_C2
        ...
        M_before = float(abs_P.sum() + abs_C1.sum() + abs_C2.sum())
        ...
        self.phase_flux_total = float(np.abs(A_eff @ P).sum() + np.abs(A_eff @ abs_C1).sum())
        ...
        if cfg.complex_classical:
            dC1_dt = (transport_C1
                      + (A_P_to_C1 - A_C1_to_P - A_C1_to_C2 + A_C2_to_C1 - A_C1_decay + A_C2_decay + A_tartini) * exp_i_thetalocal)
            dC2_dt = (transport_C2
                      + (A_C1_to_C2 - A_C2_to_C1 - A_C2_decay - A_C2_sink) * exp_i_thetalocal)
        else:
            dC1_dt = (transport_C1
        ...
        if cfg.complex_classical:
            self.C1 = self.C1 + dC1_dt * cfg.dt
            self.C2 = self.C2 + dC2_dt * cfg.dt
        else:
        ...
        abs_C1_new = np.abs(self.C1).astype(np.float32) if cfg.complex_classical else self.C1
        abs_C2_new = np.abs(self.C2).astype(np.float32) if cfg.complex_classical else self.C2

        M_after = float(abs_P_new.sum() + abs_C1_new.sum() + abs_C2_new.sum())
        ...
        if cfg.compute_lpi:
            ...
            var_C1 = float(abs_C1_new.var())
            self.lpi = var_C1 / (var_P + var_C1 + 1e-9)
        else:
            self.lpi = 0.0

        # ── 步骤8：v12.5 非厄米 PT 谱分析 ────────────────────────────
        if tick % cfg.pt_analysis_interval == 0:
            try:
                # 构造有效非厄米传播子 / ℋ_eff
                # ℋ = i D_P L_eff + D_adv A_eff - i diag(loss/|P|) + i diag(gain/|P|)
                loss_diag = (A_P_to_C1 + A_tartini) / (abs_P_new + 1e-6)
                gain_diag = (A_C1_to_P + A_C1_decay + A_C2_sink) / (abs_P_new + 1e-6)
                H_eff = 1j * cfg.D_P * L_eff + cfg.D_adv_P * A_eff - 1j * np.diag(loss_diag) + 1j * np.diag(gain_diag)
                eigvals_H = np.linalg.eigvals(H_eff)
                self.hamiltonian_eigvals = eigvals_H
                im_parts = np.imag(eigvals_H)
                # PT 破缺比例（虚部显著偏离平衡的模态数）
                self.pt_broken_ratio = float(np.mean(np.abs(im_parts) > 0.05))
                # 谱间距 (Spectral Gap)
                diffs = np.abs(eigvals_H[:, None] - eigvals_H[None, :])
                np.fill_diagonal(diffs, np.inf)
                self.pt_spectral_gap = float(np.min(diffs))
            except Exception:
                pass

        # ── 步骤9：拓扑特征种子扰动 ─────────────────────────────────
        if cfg.seed_enabled:
            M_total = abs_P_new.sum() + abs_C1_new.sum() + abs_C2_new.sum()
            f_C2_now = abs_C2_new.sum() / (M_total + 1e-9)
            if f_C2_now < cfg.seed_fc2_thresh and abs_C2_new.mean() > 1e-6:
...
class ObservationLayer:
    def __init__(self, cfg: Config):
        ...
        self.history = {k: [] for k in [
            ...
            'res_ratio', 'phase_fidelity',
            'plasticity_flux', 'degree_gini',
            'pt_broken_ratio', 'pt_spectral_gap', # v12.5 新增
        ]}
        self._hd_buf = []
    ...
    def observe(self, phys: PhysicsLayer, tick: int):
        ...
        abs_C1 = np.abs(phys.C1).astype(np.float32) if cfg.complex_classical else phys.C1
        abs_C2 = np.abs(phys.C2).astype(np.float32) if cfg.complex_classical else phys.C2
        eps = 1e-9

        M_P, M_C1, M_C2 = abs_P.sum(), abs_C1.sum(), abs_C2.sum()
        ...
        phi_C2 = abs_C2 / (abs_P + abs_C1 + abs_C2 + eps)
        ...
        h['plasticity_flux'].append(phys.plasticity_flux)
        h['degree_gini'].append(phys.degree_gini)
        h['pt_broken_ratio'].append(phys.pt_broken_ratio)
        h['pt_spectral_gap'].append(phys.pt_spectral_gap)
        return phase, C_var
...
def render(phys: PhysicsLayer, obs: ObservationLayer, tick: int):
    ...
    ax1.set_title(f'关系度规 K_eff (<K>={phys.mean_eff_coupling:.4f})\n(复数 Hebbian 拓扑学习)', color='white', fontsize=7, pad=3)
    ax1.axis('off')
    ...
    ax2.legend(loc='upper right', fontsize=5, facecolor='#141414', labelcolor='white')
    ax2.set_xlim(-1.2, 1.2)
    ax2.set_ylim(-1.2, 1.2)
    ax2.tick_params(colors='white', labelsize=6)
    ax2.set_title('P 场量子相图与相干共振矢', color='white', fontsize=7, pad=3)

    # 3. 非厄米 PT 谱图 (Re(λ) vs Im(λ))
    ax3 = fig.add_subplot(gs[0, 2])
    ax3.set_facecolor('#141414')
    if len(phys.hamiltonian_eigvals) > 0:
        ax3.scatter(np.real(phys.hamiltonian_eigvals), np.imag(phys.hamiltonian_eigvals),
                    c='#00ffcc', s=16, alpha=0.85, edgecolors='none')
        ax3.axhline(0, color='#666666', lw=0.6, linestyle='--')
        ax3.axvline(0, color='#666666', lw=0.6, linestyle='--')
    ax3.tick_params(colors='white', labelsize=6)
    ax3.set_xlabel('Re(λ)', color='white', fontsize=7)
    ax3.set_ylabel('Im(λ)', color='white', fontsize=7)
    ax3.set_title(f'非厄米 PT 谱分布\n(破缺率={phys.pt_broken_ratio:.2f}, Gap={phys.pt_spectral_gap:.3f})', color='white', fontsize=7, pad=3)

    # 4. 节点度强度分布
    ax4 = fig.add_subplot(gs[0, 3])
    ax4.set_facecolor('#141414')
    node_idx = np.arange(cfg.N_NODES)
    deg_arr = np.abs(phys.K_eff_vis).sum(axis=1)
    ax4.bar(node_idx, np.sort(deg_arr)[::-1], color='#e67e22', width=0.8)
    ax4.tick_params(colors='white', labelsize=6)
    ax4.set_xlabel('降序节点', color='white', fontsize=7)
    ax4.set_title(f'节点度强度分布 (Gini={phys.degree_gini:.3f})', color='#aaaaaa', fontsize=7, pad=3)

    # 5. 三态幅值分布
    ax5 = fig.add_subplot(gs[0, 4])
    ax5.set_facecolor('#141414')
    ax5.plot(node_idx, abs_P,  color='#3498db', lw=1.0, label='|P|')
    ax5.plot(node_idx, abs_C1, color='#e67e22', lw=1.0, label='|C1|')
    ax5.plot(node_idx, abs_C2, color='#2ecc71', lw=1.0, label='|C2|')
    ax5.legend(loc='upper right', fontsize=5, facecolor='#141414', labelcolor='white')
    ax5.tick_params(colors='white', labelsize=6)
    ax5.set_xlabel('节点序号', color='white', fontsize=7)
    ax5.set_title('三态标量幅值分布', color='#aaaaaa', fontsize=7, pad=3)
    ...
    if _n > 1000:
        ...
        h_plot = {k: [h[k][i] for i in _idx] for k in
                  ['f_C2', 'f_C1', 'f_P', 'C_var', 'Phi_net', 'Theta_barrier',
                   'lpi', 'kuramoto_r', 'mean_eff_coupling', 'degree_gini',
                   'plasticity_flux', 'pt_broken_ratio', 'phase']}
    else:
        ticks_plot = ticks
        h_plot = h

    ax_ts = fig.add_subplot(gs[1, :3])
    ax_ts.set_facecolor('#141414')
    ...
    ax_ts.legend(loc='upper left', fontsize=6, facecolor='#141414', labelcolor='white', framealpha=0.6)
    ax_ts.tick_params(colors='white', labelsize=7)
    ax_ts.set_xlabel('演化步数 (tick)', color='white', fontsize=8)
    ax_ts.set_title('终态总装宇宙时序演化', color='#aaaaaa', fontsize=8, pad=4)

    # 关系拓扑演化时序
    ax_net = fig.add_subplot(gs[1, 3])
    ax_net.set_facecolor('#141414')
    ax_net.plot(ticks_plot, h_plot['mean_eff_coupling'], color='#e67e22', lw=1.0, label='平均耦合 <K>')
    ax_net.plot(ticks_plot, h_plot['degree_gini'],        color='#f39c12', lw=1.0, label='关系基尼系数')
    ax_net.plot(ticks_plot, h_plot['plasticity_flux'],   color='#e91e63', lw=1.0, label='赫布塑形通量')
    ax_net.legend(loc='upper left', fontsize=6, facecolor='#141414', labelcolor='white')
    ax_net.tick_params(colors='white', labelsize=7)
    ax_net.set_xlabel('tick', color='white', fontsize=8)
    ax_net.set_title('网络自组织拓扑动力学', color='#aaaaaa', fontsize=8, pad=4)

    # Kuramoto 与 PT 破缺时序
    ax_pt = fig.add_subplot(gs[1, 4])
    ax_pt.set_facecolor('#141414')
    ax_pt.plot(ticks_plot, h_plot['kuramoto_r'],     color='#00ffff', lw=1.0, label='Kuramoto r')
    ax_pt.plot(ticks_plot, h_plot['pt_broken_ratio'], color='#ff0055', lw=1.0, label='PT 破缺率')
    ax_pt.legend(loc='upper left', fontsize=6, facecolor='#141414', labelcolor='white')
    ax_pt.tick_params(colors='white', labelsize=7)
    ax_pt.set_title('相干性与非厄米 PT 动力学', color='#aaaaaa', fontsize=8, pad=4)

    phase_now = h['phase'][-1] if h['phase'] else '—'
    color_now = PHASE_COLORS.get(phase_now, 'white')
    fig.suptitle(
        f'涌积态宇宙 v12.5（全复数终态总装）   第 {tick:05d} 帧   '
        f'N=128   Kuramoto r={phys.kuramoto_r:.3f}   度Gini={phys.degree_gini:.3f}   '
        f'PT破缺率={phys.pt_broken_ratio:.2f}   阶段：{phase_now}',
...
def run():
    ...
    print("=" * 70)
    print("涌积态宇宙模型 v12.5 — 全复数终态总装架构启动")
    print(f"  节点规模: N={cfg.N_NODES}   dt={cfg.dt}   最大帧数={cfg.steps}")
    print(f"  学习参数: eta_H={cfg.eta_hebbian}   rho={cfg.rho_hebbian}   beta={cfg.beta_tartini}")
    print(f"  非厄米 PT 谱分析: 开启 (每 {cfg.pt_analysis_interval} 步)")
    print(f"  经典层复数实验开关: {'开启 (COMPLEX)' if cfg.complex_classical else '标准实数层 (REAL + LOCAL_PHASE)'}")
    print(f"  关系谱半径: lambda_max={phys.max_spectral_radius:.4f} (稳定卫兵开启)")
    ...
    for tick in range(cfg.steps):
        ...
        if tick % 500 == 0 and tick > 0:
            ...
            print(f'  [progress] {pct:5.1f}%  tick={tick:06d}  '
                  ...
                  f'PT_bk={h["pt_broken_ratio"][-1]:.2f}  '
                  f'<K>={h["mean_eff_coupling"][-1]:.3f}  '
                  f'degGini={h["degree_gini"][-1]:.3f}  '
                  f'LPI={h["lpi"][-1]:.3f}')
...
def _print_summary(obs: ObservationLayer, tuner=None):
    ...
    print(f"  最终 PT 破缺率:  {h['pt_broken_ratio'][-1]:.4f}")
    print(f"  最终平均耦合 <K>: {h['mean_eff_coupling'][-1]:.4f}")
    print(f"  最终关系基尼:    {h['degree_gini'][-1]:.4f}")
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    mean_k    = np.array(h['mean_eff_coupling'])
    deg_g     = np.array(h['degree_gini'])
    flux_h    = np.array(h['plasticity_flux'])
    pt_bk     = np.array(h['pt_broken_ratio'])
    pt_gap    = np.array(h['pt_spectral_gap'])
    ...
    for pct in range(0, 101, 10):
        ...
        milestones.append({
            ...
            'km_r': float(km_r[idx]), 'mean_k': float(mean_k[idx]), 'deg_g': float(deg_g[idx]),
            'pt_bk': float(pt_bk[idx]), 'res': float(res_r[idx]), 'theta': float(theta_arr[idx]), 'phase': phases[idx],
        })

    lines = []
    lines.append("# 涌积态宇宙模型 v12.5 — 全复数终态总装运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    lines.append(f"**输出目录**：`{OUTPUT_DIR}`  ")
    lines.append(f"**RNG 种子**：`{cfg.rng_seed}`  ")
    lines.append(f"**赫布学习率 η_H**：`{cfg.eta_hebbian}`  ")
    lines.append(f"**塔蒂尼耦合系数 β**：`{cfg.beta_tartini}`  ")
    lines.append(f"**自相位调制系数 γ**：`{cfg.gamma_spm}`  ")
    lines.append(f"**经典层复数模式**：`{cfg.complex_classical}`\n")
    lines.append("---\n")
    lines.append("## 一、终态总装参数总表\n")
    lines.append("| 参数 | 值 | 说明 |")
    lines.append("|---|---|---|")
    lines.append(f"| 节点数 N | {cfg.N_NODES} | 全复数量子-关系场 |")
    lines.append(f"| dt / steps | {cfg.dt} / {cfg.steps} | 时步与离散帧数 |")
    lines.append(f"| rng_seed | {cfg.rng_seed} | 确定性随机种子 |")
    lines.append(f"| eta_hebbian | {cfg.eta_hebbian} | 赫布拓扑塑形速率 |")
    lines.append(f"| rho_hebbian | {cfg.rho_hebbian} | 拓扑关系耗散衰减率 |")
    lines.append(f"| beta_tartini | {cfg.beta_tartini} | 塔蒂尼量子非线性耦合 |")
    lines.append(f"| gamma_spm | {cfg.gamma_spm} | 塔蒂尼自相位调制强度 (SPM) |")
    lines.append(f"| complex_classical | {cfg.complex_classical} | 经典层复数实验模式 |")
    lines.append(f"| pt_analysis_interval | {cfg.pt_analysis_interval} | 非厄米谱分析采样周期 |")
    ...
    lines.append(f"| 最终 PT 破缺比例 | {pt_bk[-1]:.4f} |")
    lines.append(f"| 最终平均关系强度 <K> | {mean_k[-1]:.4f} |")
    lines.append(f"| 最终关系基尼系数 | {deg_g[-1]:.4f} |")
    lines.append(f"| 最终共振比例 | {res_r[-1]:.4f} |")
    lines.append(f"| 最终 LPI | {lpi_arr[-1]:.4f} |")
    lines.append(f"| C_var 峰值 | {float(c_var[cvar_peak_idx]):.5f} |")
    ...
    lines.append("| 进度 | tick | f_P | f_C1 | f_C2 | C_var | LPI | Kuramoto r | PT破缺 | <K> | 度Gini | 共振率 | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i, m in enumerate(milestones):
        lines.append(
            f"| {i*10}% | {m['tick']} | {m['f_P']:.4f} | {m['f_C1']:.4f} | {m['f_C2']:.4f} | "
            f"{m['C_var']:.5f} | {m['lpi']:.4f} | {m['km_r']:.4f} | {m['pt_bk']:.2f} | {m['mean_k']:.4f} | {m['deg_g']:.4f} | "
            f"{m['res']:.4f} | {m['phase']} |"
        )
    lines.append("\n---\n")
    lines.append("## 六、全量时序数据（每帧）\n")
    lines.append("| tick | f_P | f_C1 | f_C2 | C_var | LPI | Kuramoto r | PT破缺 | <K> | 度Gini | 共振率 | Theta | 阶段 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for i in range(len(ticks)):
        lines.append(
            f"| {int(ticks[i])} | {f_p[i]:.4f} | {f_c1[i]:.4f} | {f_c2[i]:.4f} | "
            f"{c_var[i]:.5f} | {lpi_arr[i]:.4f} | {km_r[i]:.4f} | {pt_bk[i]:.2f} | {mean_k[i]:.4f} | {deg_g[i]:.4f} | "
            f"{res_r[i]:.4f} | {theta_arr[i]:.4f} | {phases[i]} |"
        )
