"""
涌积态宇宙模型 v8.0 — 涌现度规与早期图灵激活架构
基于 v7.0（T-016 最终参数），新增两项核心升级：

  1. 涌现度规（Emergent Metric，alpha_metric > 0）
     stock_state_C2 高的区域压低 D_C1/D_C2 扩散系数，形成"引力锁定"效应。
     物理意义：信息密度极高的区域时空弯曲，物质无法扩散流出（黑洞效应）。
     预期：图灵斑图更稳定，C_var 峰值更高，图灵期持续时间更长。

  2. 早期图灵激活（Early Turing Seeding）
     v7.0 的图灵期出现时间约 tick=8200，前 8000 帧为 UNIFORM_GROWTH 过渡期。
     新增机制：在启动期（f_C2 < seed_fc2_thresh）向 C2 场注入空间种子扰动，
     打破初始对称性，让图灵不稳定性更早被激活。
     种子扰动：低频空间噪声（大尺度斑图种子），强度随 f_C2 增大而自动衰减。
...
class Config:
    ...
    W, H   = 128, 128
    dt     = 0.05
    steps  = 24000  # T-011: 超超长程验证高位态是否最终退出（日常运行用 24000）

    # ── 扩散系数 ──────────────────────────────────────────────────────
    D_P    = 0.20
    D_C1   = 0.05
    D_C2   = 0.004   # v7.0: 降低（原0.008），促进局部聚集

    # ── 传承失真 ──────────────────────────────────────────────────────
    k_noise   = 0.50   # v7.0: 提高（原0.30），打破初始对称性
    blur_size = 5
    ...
    lam_rate_C2  = 0.005
    lam_state_C1 = 0.005
    lam_state_C2 = 0.003   # v7.0 T-013: 提高（原0.001），防止长程积累锁死
    ...
    theta_sink  = 0.03   # v7.0 T-016: 最优值（0.08无效，0.005过强）
    gamma_sink  = 0.02
    sink_window = 5
    ...
    # ── [v8.0 新增] 涌现度规（Emergent Metric）───────────────────────
    # stock_state_C2 高的区域压低扩散系数（黑洞效应）
    # alpha_metric=0 时退化为 v7.0（用于基线对照）
    alpha_metric  = 0.3   # T-006: 回退（0.6反效果），0.3是当前最优

    # ── [v8.0 新增] 早期图灵种子扰动（Early Turing Seeding）─────────
    # 在启动期向 C2 场注入低频空间种子，打破初始对称性
    seed_enabled    = True    # T-006: 重新启用，守恒修复已完成
    seed_fc2_thresh = 0.40    # T-002: 提高（原0.15），让种子扰动在C2积累更长时间内持续注入
    seed_strength   = 0.002   # 种子扰动强度（相对于 C2 均值的比例）
    seed_freq       = 2       # T-007: 降低（原4），更低频大尺度种子，减少对图灵斑图的干扰


# ─────────────────────────────────────────────
# 二、自适应调参模块（继承 v7.0，规则7/7b 保持禁用）
# ─────────────────────────────────────────────
...
def div_diff(field, D_field):
    """守恒的非均匀扩散：∇·(D(x,y)·∇C)，周期边界
    sum(div_diff) = 0，质量严格守恒。
    实现：在半格点计算通量，再取散度。
    """
    # 半格点扩散系数（相邻格点平均）
    D_xp = 0.5 * (D_field + np.roll(D_field, -1, axis=1))  # x+1/2
    D_xm = 0.5 * (D_field + np.roll(D_field,  1, axis=1))  # x-1/2
    D_yp = 0.5 * (D_field + np.roll(D_field, -1, axis=0))  # y+1/2
    D_ym = 0.5 * (D_field + np.roll(D_field,  1, axis=0))  # y-1/2
    # 通量：D * ∇C
    ...
    # 散度：(flux_xp - flux_xm) + (flux_yp - flux_ym)
...
class PhysicsLayer:
    ...
    def step(self, tick: int = 0):
        ...
        # ── 步骤6：[v8.0] 涌现度规 ───────────────────────────────────
        # stock_state_C2 高的区域压低扩散系数（黑洞效应）
        # 使用散度形式 ∇·(D(x,y)·∇C) 保证质量守恒
        S_C2_norm = self.stock_state_C2 / (self.stock_state_C2 + cfg.k_norm_base)
        metric_factor = np.maximum(0.1, 1.0 - cfg.alpha_metric * S_C2_norm)
        ...
        # ── 步骤8：[v8.0] 早期图灵种子扰动 ──────────────────────────
        # 在 f_C2 < seed_fc2_thresh 时向 C2 注入低频空间种子
        # 强度随 f_C2 增大线性衰减；乘性扰动：天然守恒，无截断问题
        # C2_new = C2 * (1 + template * strength)，始终非负，一次比例缩放即可
        if cfg.seed_enabled:
            ...
            if f_C2_now < cfg.seed_fc2_thresh and self.C2.mean() > 1e-6:
                strength = cfg.seed_strength * (1.0 - f_C2_now / cfg.seed_fc2_thresh)
                C2_sum_before = self.C2.sum()
                # 乘性扰动：scale > 0 保证非负，无需截断
                scale = 1.0 + self._seed_template * strength
                scale = np.maximum(scale, 0.01)  # 防止极端情况下 scale 为负
...
def render(phys: PhysicsLayer, obs: ObservationLayer, tick: int):
    ...
    # [v8.0] 第三格改为涌现度规场（metric_factor 可视化）
    S_C2_norm = phys.stock_state_C2 / (phys.stock_state_C2 + cfg.k_norm_base)
    metric_vis = np.maximum(0.1, 1.0 - cfg.alpha_metric * S_C2_norm)

    fields = [
        (phi_C2,           'viridis',  None,            'φ_C2  高阶结构密度\n蓝=低  黄=高'),
        (phys.I_drive,     'RdBu_r',   TwoSlopeNorm(0), 'I_drive  驱动场\n红=生长  蓝=退化  白=零'),
        (metric_vis,       'plasma',   None,            '[v8] 涌现度规\n亮=扩散正常  暗=时空弯曲锁定'),
...
def run():
    ...
    print(f"  [v7.0继承] w_rate_C2={cfg.w_rate_C2}  lam_state_C2={cfg.lam_state_C2}")
    print(f"             alpha_lambda={cfg.alpha_lambda}  theta_sink={cfg.theta_sink}")
    print(f"  [v8.0新增] alpha_metric={cfg.alpha_metric}  (涌现度规)")
    print(f"  [v8.0新增] seed_enabled={cfg.seed_enabled}  seed_strength={cfg.seed_strength}"
          f"  seed_freq={cfg.seed_freq}  seed_fc2_thresh={cfg.seed_fc2_thresh}")
    print(f"  自适应调参: {'开启' if cfg.adaptive_tuning else '关闭'}（规则7/7b禁用）")
...
def _save_report(obs: ObservationLayer, tuner=None):
    ...
    lines = []
    lines.append("# 涌积态宇宙模型 v8.0 — 运行报告")
    lines.append(f"\n**运行时间**：{_RUN_TIME}  ")
    ...
    lines.append(f"| alpha_lambda / theta_sink | {cfg.alpha_lambda} / {cfg.theta_sink} |")
    lines.append(f"| **[v8.0]** alpha_metric | {cfg.alpha_metric} |")
    lines.append(f"| **[v8.0]** seed_enabled / strength / freq | {cfg.seed_enabled} / {cfg.seed_strength} / {cfg.seed_freq} |")
    lines.append(f"| **[v8.0]** seed_fc2_thresh | {cfg.seed_fc2_thresh} |")
