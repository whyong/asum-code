"""
涌积态宇宙模型 v13.0 — 地球自然演化与生态相变引擎
(ASUM v13.0: Earth Natural Evolution & Ecological Phase Transition Engine)
...
class Config:
    ...
    gamma_drift      = 0.20    # 宏观漂移强度 γ（生态位引力强度）[P2修复: 0.12→0.20]
    ...
    lambda_eco       = 0.08    # C1 生态竞争死亡率基值 λ
    eps_carrying     = 1e-3    # 防零除 ε

    # ── [v13.0 新增] D 算子双轨 ──────────────────────────────────────
    k_noise_micro    = 0.30    # D_micro 乘性噪声强度（日常基因突变）
    p_catastrophe    = 5e-5    # D_macro 每帧触发概率（≈20000帧1次）[P3修复: 2e-5→5e-5]
    catastrophe_c1_suppress = 0.05   # 灾变后 C1 保留比例
    catastrophe_c2_suppress = 0.10   # 灾变后 C2 保留比例
