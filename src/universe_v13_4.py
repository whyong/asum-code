"""
涌积态宇宙模型 v13.4 — 地球自然演化与生态相变引擎（动态板块与地热扫频版）
(ASUM v13.4: Earth Natural Evolution & Ecological Phase Transition Engine - Dynamic Tectonic Drift & Geothermal Sweep)

  v13.4 核心升级与特性：
    【重大】新增动态板块与地热热点漂移系统（Tectonic & Hotspot Drift）：
      - 模拟地幔对流与板块慢变量位移：热点随时间缓慢漂移，周期性环形平移
      - 化能合成高地迁移 → 迫使 C1/C2 种群进行千帧尺度的生态位地质迁徙
      - 旧址残留 M* 形成地层断层记忆，新址诱发全新 Turing 斑块成核
    【重大】支持标准化环境变量注入，驱动多维参数扫频与超长时程演化实验
    继承 v13.3：地热双热源系统（70% 矿化至 P，30% 化能合成直通 C1）
    继承 v13.2：差异化废热辐射出口（sigma_rad_P > sigma_rad_C1 > sigma_rad_C2）
    继承 v13.1：M* 上限 50.0、gamma_drift=0.15、p_catastrophe=2.5e-5、lambda_eco=0.05
    继承 v13.0：Fokker-Planck 东西方双向因果、60甲子太阳泵、64卦引力盆地
...
# 支持外部指定输出子目录
_CUSTOM_OUT = os.environ.get('ASUM_OUTDIR', '')
if _CUSTOM_OUT:
    OUTPUT_DIR = _SCRIPT_DIR / _SCRIPT_NAME / _CUSTOM_OUT
else:
...
class Config:
    # ── 网格与时间 ──────────────────────────────────────────────────
    W, H   = 128, 128
    dt     = float(os.environ.get('ASUM_DT', 0.10))
    steps  = int(os.environ.get('ASUM_STEPS', 30000))
    seed   = int(os.environ.get('ASUM_SEED', 42))
    ...
    # ── 开放系统：太阳泵 ────────────────────────────────────────────
    A_solar          = float(os.environ.get('ASUM_SOLAR_AMP', 0.08))
    P_in_spatial_var = 0.30    # 阳光空间异质性
    ...
    # ── Fokker-Planck 漂移场 ────────────────────────────────────────
    gamma_drift      = float(os.environ.get('ASUM_GAMMA_DRIFT', 0.15))
    alpha_Mstar      = 1.0     # M* 在漂移势中的权重
    alpha_Istar      = 0.5     # I* 在漂移势中的权重
    Mstar_accum_rate = 0.008   # M* 积累速率
    Mstar_decay_rate = 0.9998  # M* 每帧衰减（地质时间尺度缓慢侵蚀）
    Mstar_max_clip   = 50.0    # M* 饱和上限截断

    # ── L 算子：生态容量竞争 ────────────────────────────────────────
    lambda_eco       = 0.05    # C1 生态竞争死亡率基值 λ
    eps_carrying     = 1e-3    # 防零除 ε

    # ── D 算子双轨 ──────────────────────────────────────────────────
    k_noise_micro    = 0.30    # D_micro 乘性噪声强度（日常基因突变）
    p_catastrophe    = float(os.environ.get('ASUM_P_CAT', 2.5e-5))  # D_macro 灾变概率
    catastrophe_c1_suppress = 0.05   # 灾变后 C1 保留比例
    catastrophe_c2_suppress = 0.20   # 灾变后 C2 保留比例
    ...
    # ── 废热辐射出口（向宇宙空间）──────────────────────────────────
    sigma_rad_P    = 0.006   # P 辐射率（无机游离能，高熵最快）
    sigma_rad_C1   = 0.002   # C1 辐射率（生物个体代谢热）
    sigma_rad_C2   = 0.0004  # C2 辐射率（顶极群落维持，最节能）

    # ── [v13.4 升级] 动态地热能与板块漂移系统 ───────────────────────
    geo_heat_amplitude = float(os.environ.get('ASUM_GEO_AMP', 0.004))
    geo_hotspot_count  = int(os.environ.get('ASUM_HOTSPOT_COUNT', 5))
    geo_hotspot_radius = float(os.environ.get('ASUM_HOTSPOT_RADIUS', 10.0))
    geo_to_C1_frac     = float(os.environ.get('ASUM_GEO_TO_C1', 0.30))
    # 板块漂移速度（格点/帧，地质时间尺度，默认 0.0008 格点/帧 ≈ 1250 帧移动 1 格点）
    geo_drift_speed    = float(os.environ.get('ASUM_DRIFT_SPEED', 0.0008))
    ...
    # ── 观测与渲染 ──────────────────────────────────────────────────
    render_every  = int(os.environ.get('ASUM_RENDER_EVERY', 0))
    obs_sample_every = 5       # 观测采样步长（避免每步做大量历史列表append）
    struct_every  = 200
    phi_threshold = None
    hd_window     = 300
    hd_drive_eps  = 5e-5
    ...
    experiment = os.environ.get('ASUM_EXPERIMENT', 'free')
...
class JiaziClock:
    ...
    def P_in(self, tick: int) -> np.ndarray:
        ...
        rhythm = 1.0 + cfg.jiazi_amplitude * phase / 2.0
    ...
    def hexagram_index(self, tick: int) -> int:
        t = tick * self.cfg.dt
        tg = int(t / self.cfg.tian_gan_period) % 10
        dz = int(t / self.cfg.di_zhi_period)  % 12
        tg3 = tg % 8
        dz3 = dz % 8
        return (tg3 << 3) | dz3
...
# ─────────────────────────────────────────────
# 四、动态板块与地热热点引擎 (v13.4 新增)
# ─────────────────────────────────────────────

class DynamicTectonics:
    """动态板块与地热热点漂移系统

    模拟地幔对流与板块慢变量位移：
    - 维持 K 个热点的中心坐标与漂移速度矢量
    - 每帧根据 drift_speed 缓慢演化热点位置，并支持在环面网格上平滑 wrapping
    - 输出当前帧的动态二维地热场 Geo(x, y, t)
    """
    def __init__(self, cfg: Config):
        self.cfg = cfg
        H, W = cfg.H, cfg.W
        rng = np.random.default_rng(2024 + cfg.seed % 1000)

        self.K = cfg.geo_hotspot_count
        # 初始热点中心坐标 (float)
        self.centers_x = rng.uniform(10.0, W - 10.0, self.K).astype(np.float32)
        self.centers_y = rng.uniform(10.0, H - 10.0, self.K).astype(np.float32)

        # 漂移方向与速度矢量（微小角度与漂移率）
        angles = rng.uniform(0, 2*np.pi, self.K).astype(np.float32)
        self.vx = (cfg.geo_drift_speed * np.cos(angles)).astype(np.float32)
        self.vy = (cfg.geo_drift_speed * np.sin(angles)).astype(np.float32)

        # 预先生成网格坐标网格
        self.yy, self.xx = np.ogrid[:H, :W]
        self._cached_field = None
        self._last_tick = -1

    def update_and_get_field(self, tick: int) -> np.ndarray:
        # 地热板块属于地质慢变量，每 10 帧更新一次空间场分布，其余帧直接复用缓存，极大提升计算吞吐
        if self._cached_field is not None and (tick - self._last_tick) < 10:
        ...
        # 更新热点坐标
        dt_sub = (tick - self._last_tick) if self._last_tick >= 0 else 1
        self.centers_x = (self.centers_x + self.vx * dt_sub) % W
        self.centers_y = (self.centers_y + self.vy * dt_sub) % H
        self._last_tick = tick

        field = np.zeros((H, W), dtype=np.float32)
        inv_2sigma2 = 1.0 / (2.0 * (cfg.geo_hotspot_radius ** 2))
        ...
        for k in range(self.K):
            ...
            dx = np.abs(self.xx - cx)
            np.minimum(dx, W - dx, out=dx)

            dy = np.abs(self.yy - cy)
            np.minimum(dy, H - dy, out=dy)

            dist2 = dx**2 + dy**2
            field += np.exp(-dist2 * inv_2sigma2)

        # 归一化并叠加全球均匀地温底噪
        peak = field.max() + 1e-9
        field = field / peak * cfg.geo_heat_amplitude
        field += cfg.geo_heat_amplitude * 0.15
        self._cached_field = field.astype(np.float32)
        return self._cached_field
...
# ─────────────────────────────────────────────
# 六、物理层（v13.4 动态板块生态引擎）
# ─────────────────────────────────────────────

class PhysicsLayer:
    def __init__(self, cfg: Config, clock: 'JiaziClock', tectonics: 'DynamicTectonics'):
        self.cfg       = cfg
        self.clock     = clock
        ...
        # ── 记忆场 M* 与多样性场 I* ─────────────────────────────────
        self.M_star = np.zeros(shape, np.float32)
        self.I_star = np.zeros(shape, np.float32)
        ...
        self.geo_field  = np.zeros(shape, np.float32)
        self.geo_flux   = 0.0
        self.rad_total  = 0.0

        # ── 灾变状态 ──────────────────────────────────────────────────
        self.catastrophe_active    = False
        self._catastrophe_cooldown = 0
    ...
    def _apply_experiment_init(self):
        cfg = self.cfg
        shape = (cfg.H, cfg.W)
        rng   = np.random.default_rng(99 + cfg.seed % 100)
    ...
    def step(self, tick: int, c_var: float = 0.0, phase: str = 'CHAOTIC_EDGE'):
        ...
        # [v13.4 Step-A] M* 记忆场更新 (dt 时间尺度归一化)
        decay_factor = 1.0 - (1.0 - cfg.Mstar_decay_rate) * (cfg.dt / 0.05)
        self.M_star = (self.M_star * decay_factor
                       + cfg.Mstar_accum_rate * C2 * cfg.dt).astype(np.float32)
        np.clip(self.M_star, 0.0, cfg.Mstar_max_clip, out=self.M_star)
        ...
        # [v13.4 Step-C] Fokker-Planck 漂移场
        ...
        # [v13.4 Step-E] 动态板块地热场计算
        self.geo_field = self.tectonics.update_and_get_field(tick)
        self.geo_flux  = float(self.geo_field.sum()) * cfg.dt

        # [v13.4 Step-F] D_macro 泊松灾变脉冲
        self.catastrophe_active = False
        if (not self.catastrophe_active
            ...
            self.C1 = (C1 * cfg.catastrophe_c1_suppress
                       + np.random.exponential(0.02, C1.shape)).astype(np.float32)
            self.C2 = (C2 * cfg.catastrophe_c2_suppress).astype(np.float32)
        ...
        ctx_factor = np.exp(-1.5 * grad_mag / (self._I_macro_prev + 1e-6)).astype(np.float32)
        ...
        # 批量生成特征与随机掩码（极大提升 NumPy 向量化吞吐）
        chis = (chi_rate_P, chi_rate_C1_eff, chi_rate_C2_eff, chi_state_C1, chi_state_C2)
        k_ws = (cfg.kw_rate_P, cfg.kw_rate_C1, cfg.kw_rate_C2, cfg.kw_state_C1, cfg.kw_state_C2)
        lams = (lam_rP, lam_rC1, lam_rC2, lam_sC1, lam_sC2)
        weights = (cfg.w_rate_P, cfg.w_rate_C1, cfg.w_rate_C2, cfg.w_state_C1, cfg.w_state_C2)
        stocks = (self.stock_rate_P, self.stock_rate_C1, self.stock_rate_C2, self.stock_state_C1, self.stock_state_C2)

        I_macro = np.zeros_like(P)
        I_macro_rate_sum = np.zeros_like(P)

        rand_arrs = np.random.random((5, *P.shape)).astype(np.float32)

        for i in range(5):
            chi = chis[i]
            p_w = chi / (chi + k_ws[i])
            mask = (rand_arrs[i] < p_w).astype(np.float32)
            stocks[i][:] = stocks[i] * (1.0 - lams[i]) + chi * mask * cfg.dt
            ns = stocks[i] / (stocks[i] + cfg.k_norm_base)
            I_macro += weights[i] * ns
            if i < 3:
                I_macro_rate_sum += weights[i] * ns
        ...
        metric_factor = np.maximum(0.1, 1.0 - cfg.alpha_metric*S_C2_norm - cfg.alpha_metric_C1*S_C1_norm)
        ...
        eco_decay_C1   = cfg.lambda_eco * C1 * np.exp(np.clip(carrying_ratio - 1.0, -3.0, 3.0))
        ...
        # PDE 更新方程
        dP_dt = (cfg.D_P  * lap(P)
                 + advection_upwind(P, self.Vx, self.Vy)
                 + self.P_in_field
                 + geo_to_P
                 - A_P_to_C1 + A_C1_to_P
                 + eco_decay_C1
...
class ObservationLayer:
    ...
    def observe(self, phys: PhysicsLayer, tick: int, clock: JiaziClock):
        ...
        # 降采样记录历史
        if tick % cfg.obs_sample_every == 0 or tick == cfg.steps - 1:
            h = self.history
            h['tick'].append(tick)
...
def render(phys: PhysicsLayer, obs: ObservationLayer,
    ...
    fields_top = [
        ...
        (phys.geo_field, 'hot',     'Geo(x,y,t) 动态地热板块\n(漂移热点·化能合成高地)'),
    ...
    ax_ts.set_title('地球生态演化时序 (背景=阶段 红虚=灾变)',
    ...
    ax_sol.plot(tp, geo_vals, color='#e67e22', lw=1.2, linestyle='--', label='动态地热')
    ...
    ax_sol.set_title('能量收支平衡 (太阳+地热 vs 辐射)',
    ...
    fig.suptitle(
        f'涌积态地球生态引擎 v13.4 [{cfg.experiment.upper()}]   第 {tick:05d} 帧   '
...
def run():
    ...
    tectonics = DynamicTectonics(cfg)
    phys      = PhysicsLayer(cfg, clock, tectonics)
    obs       = ObservationLayer(cfg)
    tuner     = AdaptiveTuner(cfg) if cfg.adaptive_tuning else None

    print('=' * 72)
    print('涌积态宇宙模型 v13.4 — 地球自然演化与生态相变引擎（动态板块扫频版）')
    print(f'  实验预设  : {cfg.experiment.upper()}   随机种子={cfg.seed}')
    print(f'  网格/步数 : {cfg.W}x{cfg.H}   dt={cfg.dt}   总步数={cfg.steps}')
    print(f'  动态地热  : amp={cfg.geo_heat_amplitude}  热点数={cfg.geo_hotspot_count}  漂移率={cfg.geo_drift_speed}')
    ...
    for tick in range(cfg.steps):
        ...
        if cfg.render_every > 0 and tick % cfg.render_every == 0:
    ...
    print('\n模拟结束。')
    _save_summary(obs, tuner, cfg)


def _save_summary(obs: ObservationLayer, tuner=None, cfg=None):
    h = obs.history
    if not h['tick']:
        return
    ...
    print(f'  M* 峰值  : {max(h["M_star_mean"]):.5f}')
    print(f'  I* 峰值  : {max(h["I_star_mean"]):.5f}')
    print(f'  灾变次数 : {cat_cnt}')
    print(f'  最终阶段 : {h["phase"][-1]}')
    print('──────────────────────────────────────────────────────')

    lines = [
        f'# 涌积态地球生态引擎 v13.4 [{cfg.experiment.upper()}] — 模拟报告',
        f'\n**运行时间**：{_RUN_TIME}',
        f'**输出目录**：`{OUTPUT_DIR}`',
        f'**实验参数**：steps={cfg.steps}, geo_amp={cfg.geo_heat_amplitude}, hotspot_count={cfg.geo_hotspot_count}, drift_speed={cfg.geo_drift_speed}\n',
        '---\n',
        '## 生态演化摘要\n',
        '| 指标 | 值 |', '|---|---|',
        f'| 总帧数 | {h["tick"][-1]} |',
