"""
涌积态宇宙模型 v13.2 — 地球自然演化与生态相变引擎（热力学完备版）
(ASUM v13.2: Earth Natural Evolution & Ecological Phase Transition Engine - Thermodynamically Complete)

  v13.2 核心修正（基于热力学平衡原理）：
    【重大】新增废热辐射出口：P/C1/C2 以不同速率向宇宙空间辐射废热
      sigma_rad_P  = 0.006  高熵无机能，辐射最快（热沙漠/裸岩）
      sigma_rad_C1 = 0.002  代谢热损耗，中等（动物呼吸/植物蒸腾）
      sigma_rad_C2 = 0.0004 低熵有序结构，辐射最慢（雨林/珊瑚礁）
    → 三态辐射率差异 = 进化方向的热力学驱动力（C2最节能→自然选择）
    → 太阳泵输入 ≈ 三态废热辐射之和 → 真正的热力学耗散稳态
  v13.1 参数继承：
    - gamma_drift=0.15  p_catastrophe=2.5e-5  c2_suppress=0.20  lambda_eco=0.05
  v13.2 追加调整：
    - Theta_barrier 0.28 → 0.18  开放系统势垒降低，释放C1→C2跃迁
    - Base_Rate_2   0.40 → 0.60  跃迁速率提升，配合低势垒
    - Mstar_accum_rate 0.005→0.008 加快地层沉积，增强记忆引力
...
class ObservationLayer:
    def __init__(self, cfg: Config):
        ...
        self.history = {k: [] for k in [
            ...
            'catastrophe','hexagram_idx','rad_total','energy_balance',
        ]}
        self._hd_buf = []
    ...
    def observe(self, phys: PhysicsLayer, tick: int, clock: JiaziClock):
        ...
        h['energy_balance'].append(solar_in - phys.rad_total)  # >0输入盈余，<0辐射盈余
        return phase, C_var
...
def render(phys: PhysicsLayer, obs: ObservationLayer,
    ...
    if _n > 1000:
        ...
        hp = {k: [h[k][i] for i in _idx] for k in
              ['f_C2','f_C1','f_P','C_var','Theta_barrier',
               ...
               'catastrophe','rad_total','energy_balance']}
    else:
        tp = tk; hp = h
    ...
    ax_sol.plot(tp, hp['solar_flux'], color='#f1c40f', lw=1.0, label='太阳泵输入')
    ...
    solar_now  = h['solar_flux'][-1] if h['solar_flux'] else 0.0
    ...
    fig.suptitle(
        f'涌积态地球生态引擎 v13.2 [{cfg.experiment.upper()}]   第 {tick:05d} 帧   '
        f'f_P={h["f_P"][-1]:.3f}  f_C1={h["f_C1"][-1]:.3f}  f_C2={h["f_C2"][-1]:.3f}  '
        f'M*={mstar_now:.4f}  废热={rad_now:.1f}  收支={bal_sign}{bal_now:.2f}',
    ...
    print(f'  [render] tick={tick:05d}  {phase_now:<18s}  '
          ...
          f'废热={rad_now:.2f}  收支={bal_sign}{bal_now:.3f}')
...
def run():
    ...
    print('=' * 72)
    print('涌积态宇宙模型 v13.0 — 地球自然演化与生态相变引擎')
