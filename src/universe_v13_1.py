"""
涌积态宇宙模型 v13.1 — 地球自然演化与生态相变引擎（创造性毁灭调优版）
(ASUM v13.1: Earth Natural Evolution & Ecological Phase Transition Engine - Creative Destruction Tuned)

  v13.1 调优说明（基于第二次运行分析）：
    - gamma_drift   0.20 → 0.15  漂移过强导致C1超载，折中取值
    - p_catastrophe 5e-5 → 2.5e-5 6次灾变过频，恢复≈40000帧1次
    - c2_suppress   0.10 → 0.20  灾变后C2保留20%，提供复苏种子
    - lambda_eco    0.08 → 0.05  降低竞争死亡率，给C1→C2跃迁更多时间
  目标：实现"灭绝-复苏-繁荣-再灭绝"的完整生态循环
...
class Config:
    ...
    Mstar_accum_rate = 0.005   # M* 积累速率（C2→M* 沉积）
...
def render(phys: PhysicsLayer, obs: ObservationLayer,
    ...
    if _n > 1000:
        ...
        hp = {k: [h[k][i] for i in _idx] for k in
              ['f_C2','f_C1','f_P','C_var','Theta_barrier',
               'M_star_mean','I_star_mean','solar_flux','phase','catastrophe']}
    else:
        tp = tk; hp = h
    ...
    ax_sol.plot(tp, hp['solar_flux'], color='#f1c40f', lw=1.0)
    ax_sol.tick_params(colors='white', labelsize=7)
    ax_sol.set_title('60甲子太阳泵  P_in(x,y,t)',
                     color='#aaaaaa', fontsize=8, pad=4)
    ...
    fig.suptitle(
        f'涌积态地球生态引擎 v13.0 [{cfg.experiment.upper()}]   第 {tick:05d} 帧   '
        f'f_P={h["f_P"][-1]:.3f}  f_C1={h["f_C1"][-1]:.3f}  '
        f'f_C2={h["f_C2"][-1]:.3f}  太阳泵={solar_now:.1f}  M*={mstar_now:.4f}',
